...
matches the **issue a test recorded**, not the test's name, so the same test still counts when it
fails on a real assertion and does not when it loses a race. A mutant with nothing else against it
comes back **UNATTRIBUTED**, which exits 2 like a survivor rather than passing as a kill; a baseline
red purely on declared failures now prints them and carries on instead of costing the whole run.
...
declaration still matches something under `Tests/`; the other direction is judgment.
