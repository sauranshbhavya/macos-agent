...
- Commits and pushes to a ticket's branch are pre-authorized for the session implementing it — no per-commit approval needed. Opening a PR is fine. **Merging is the user's, always** — never merge, and never rewrite pushed history on `main` or on any branch that is not the session's own ticket branch. The force-push a rebase requires is the exception, and it is **already authorized rather than granted here**: `git push --force-with-lease` on the session's own ticket branch, never bare `--force` and never another branch — `WORKFLOW.md`'s merge-one-branch-at-a-time rule states it in full. Commit titles reference the ticket identifier (e.g. `fix(core): SONNY-12 ...`).
