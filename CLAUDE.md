...
- Commits and pushes to a ticket's branch are pre-authorized — no per-commit approval needed. Opening a PR is fine. **Merging is the user's, always** — never merge, and never rewrite pushed history. Commit titles reference the ticket identifier (e.g. `fix(core): SON-12 ...`).
