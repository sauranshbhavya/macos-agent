...
- All 11 local stores (routines, workspaces, clipboard history + settings, snippets, recent artifacts, Shortcut run history, task history, the vision session journal, what past tasks planned, row J's approved apps) share one DI/encryption/legacy-plaintext-migration pattern via `LocalStorageEncryption`. A new store should follow it, not invent a variant. A new store is also four other things, each enforced by a test that fails until it is done: a URL in `LocalDataDeletionService.defaultStoreFileURLs()`, a case in `LocalStore` with a trace/artifact classification, a `LocalStore.memoryCategory` answer placing it under a Command Center Memory row or excluding it by name (SONNY-208 — a store nobody can see is a store the Memory section silently omits), and a line in the wipe's *behavioural* fixture — the URL list can be right while nothing ever exercises it (SONNY-154).
