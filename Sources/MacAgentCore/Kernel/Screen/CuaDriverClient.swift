...
public struct CuaCapabilityManifest: Equatable, Sendable {
    ...
    /// The tools the kernel's screen capability uses. Observation and input only: no launching, no
    /// quitting, no clipboard, no browser tools, no recording.
    public static let screenTools = [
    ...
    /// A ceiling for one task's work in one app (V2 plan section 12): cua may touch that app and
    /// nothing else.
    public static func forApp(_ bundleIdentifier: String) -> CuaCapabilityManifest {
        CuaCapabilityManifest(tools: screenTools, bundleIdentifiers: [bundleIdentifier])
    }
...
public enum CuaAction: Equatable, Sendable {
    ...
    case menu([String])
    /// Types at whatever has keyboard focus in the window.
    case typeAtFocus(String)
}

/// What cua said an action did, kept as the action's local evidence.
public struct CuaActionReport: Equatable, Sendable {
    public let effect: String?
    public let status: String?

    public var evidence: String {
        switch (effect, status) {
        case (let effect?, _): "cua reported \(effect)"
        case (nil, let status?): "cua reported \(status)"
        case (nil, nil): "cua reported no detail"
        }
    }
}
...
public struct CuaDriverClient: Sendable {
    ...
    /// Runs one action and returns cua's own account of it, which becomes the action's local
    /// evidence. The arguments are built here from the action alone: nothing the model wrote can
    /// reach cua's options (the delivery mode among them), only the action's target and text.
    @discardableResult
    public func perform(_ action: CuaAction, pid: pid_t, windowID: Int) async throws -> CuaActionReport {
        ...
        switch action {
        case .click(let ref): result = try await call("click", element(ref))
        case .doubleClick(let ref): result = try await call("double_click", element(ref))
        case .rightClick(let ref): result = try await call("right_click", element(ref))
        case .setValue(let ref, let text): result = try await call("set_value", element(ref).merging(["value": text]) { $1 })
        case .typeText(let ref, let text): result = try await call("type_text", element(ref).merging(["text": text]) { $1 })
        case .typeAtFocus(let text): result = try await call("type_text", base.merging(["text": text]) { $1 })
        case .pressKey(let key): result = try await call("press_key", base.merging(["key": key]) { $1 })
        case .hotkey(let keys): result = try await call("hotkey", base.merging(["keys": keys]) { $1 })
        case .scroll(let ref, let direction):
            result = try await call("scroll", (ref.map(element) ?? base).merging(["direction": direction]) { $1 })
        case .menu(let path): result = try await call("invoke_menu", base.merging(["path": path]) { $1 })
        }
        return CuaActionReport(effect: result["effect"] as? String, status: result["status"] as? String)
    }
