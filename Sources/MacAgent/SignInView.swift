...
final class SonnyAccountModel: ObservableObject {
    ...
    ///
    /// **The wait ends on cancellation now, and closing the sheet still does not cancel it.**
    /// `awaitPendingRefresh()` used to be `await refreshTask?.value`, which has no cancellation
    /// point; since SONNY-442 (PR #229's F1) it returns as soon as the waiting task is cancelled,
    /// because the screen-control door waits on it too and a stop there must land at once. Nothing
    /// here cancels the task this runs in, so for this dialog the bound is unchanged and it is the
    /// client's: `refreshNow()` uses `SonnyBackendTimeouts.auth` (20 s) and is not retried on a
    /// transport timeout, so roughly 20 s realistically and about 100 s worst case across the
    /// retryable codes' three attempts. The *refresh* is never cancelled by anyone — that task is
    /// detached by design.
    ///
