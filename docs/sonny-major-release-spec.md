...
Workspace scope verdict as an input (§6.9, added v1.2; relaxation shipped v1.2, row C — amended 2026-08-13):

The scope verdict for the task's bound workspace is an input to these rules alongside the tier, and it now works in **both** directions. An `outOfScope` resource still escalates to tier 3 and names what it found (§11.3). And being inside the boundary finally pays for its cost: **relaxation ships**, as two grants resolved by **one** exhaustive function of `(effectiveTier, grant)` on the policy — `RiskApprovalPolicy.requirement(for:context:)`, the sole public path to an approval requirement:

- **`inScopeWorkspace`** — the plan-level roll-up is `inScope`: tier 2 auto-runs (unless the user's own tier-2 policy says preview-only — relaxation never overrides the user's own tightening), and tier 3 drops to a lightweight confirmation.
- **`directUserAuthored`** — the plan came from `PreparedPlanSource.directUserAction`, built field by field on a UI surface with nothing interpreted on the way (the workspace detail sheet is the first caller). Same mapping. This grant exists because a workspace edit's contents are *declared*, not touched, so a sheet edit can never roll up `inScope` — the verdict grant structurally cannot reach it. Contained by a per-operation eligibility classification with no default case (only `edit_workspace` is allowlisted; routines and Shortcuts are eligible for *neither* grant — the per-routine trust toggle stays the single door for skipping a routine's prompt), and a **boundary-changing** edit — first entry into an empty dimension, a removal that empties one, an added folder that subsumes an existing entry — drops this grant and prompts as before. Adding a whitelist root itself (`~/Desktop`, `~/Documents`) as a file location escalates to tier 3 with a reason naming the consequence.

The two constraints recorded here in advance are now **satisfied, not pending**, and remain hard:

- **Relaxation is a requirement override that never writes `effectiveTier`, `escalations`, or `approvalCopy`.** The tier half is the original constraint: the field's consumers break if it lies — the unattended-run gate, the stale-approval re-check, the `risk.assessed`/`risk.escalated` trace, the unattended-trust advisory, and (a fifth gate the original list of four missed) the manual-routine-trust check that mirrors the execute gate; the full consumer population is far larger (151 references across 24 files when row C was planned). The `escalations`/`approvalCopy` half is the **strengthened invariant, added by row C**: escalation reason strings are rendered independently on three surfaces (the widget's first-run explainer, Command Center's approval panel, the scheduled-pause notice), so a relaxation that kept the tier honest while changing what is in that array would still change what the user reads. Relaxation changes the *weight* of the ask, never the sentence.
- **The tier-2 and tier-3 rules are one function, never two chained rules.** Shipped exactly so: one 15-case switch over `(effectiveTier, grant)` with no default clause; Safe mode returns before the grant is computed; and no public function takes a requirement and returns a different one, so a post-hoc clamp cannot be added without breaking the stated rule. Per-app control consent (row I) and Safe mode (row H) land as *inputs* to this same function, never as rules applied to its output.

Two further properties stand as written: only `inScope` is ever eligible for the verdict grant — `unconstrained` (the kind is unconfigured) and `opaque` (Sonny cannot see the resource) never qualify, `outOfScope` never qualifies, and a workspace-less task has no verdict at all; collapsing any of these into `inScope` "for simplicity" silently grants relaxed gating to users who never configured scope. And scope answers "right place", not "right severity" (§6.9): the genuinely irreversible tier-3 capabilities (send, upload, delete, submit, purchase, share — §11.1) still do not exist, so today's tier-3 clause governs escalated tier 3 only, and the per-operation eligibility classification is the fail-closed home for their non-relaxable floor when they arrive — a new operation cannot land unclassified.

One reachability note recorded so nobody mistakes it for a guarantee: the unattended scheduled path reaches neither grant because its dispatch makes two independent call-site choices (`source: .instantResolver`, `scope: .unscoped`) — a pinned property with a named test, not a type-level impossibility.
...
Relaxation and copy (added v1.2, row C — 2026-08-13). **Relaxation changes the *weight* of an ask, never its sentence.** A grant maps a requirement to a lighter one; it must not touch the action description, the risk reason, the involved resource, or any escalation reason — those sentences are computed from the assessment, which relaxation never writes. Two consequences for surfaces:

- `lightweightConfirmation` and `explicitApproval` are **two different requirements the approval surfaces are expected to distinguish** — a light ask and a heavy one are different consents. Honest note: **they do not yet.** Every surface renders them identically today; `feature/approval-relaxation-surface` (branch 2 of row C) builds the distinction, and until it ships the in-scope tier-3 half of the relaxation charter has no user-visible effect.
- A run that executed without asking *because a grant made it silent* carries a one-line ran-without-asking trace on the widget's result panel, naming the grant's reason (the workspace, or that the user built the action on screen). A step that would have auto-run anyway — tier 0/1 — never traces, so the signal stays meaningful; a run that prompted was disclosed by the prompt and never traces either.
