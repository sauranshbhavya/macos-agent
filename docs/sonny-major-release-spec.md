...
**Per-app control ordering, added 2026-08-21 (row J, `feature/app-control-consent`, PR #88's fix round) — read this before the note below it.** The per-app control approval is asked **inside the vision session, after its first capture has passed the terminal screen check**, and not at the plan gate. Founder decision, 2026-08-21, resolving a conflict between two of his own, both in `docs/sonny-row-j-plan.md` — every §4.3 and §2.3 in this paragraph is that plan's, not this document's, which has no §4.3 at all and whose own §2.3 is Strategic Wedge (the file was left unnamed until SONNY-290 named it on 2026-08-26): §4.3 requires that "the first capture happens before the control approval", so that a user is never asked to approve an app whose window is showing a shell — the accidental-approval trap SONNY-102 names, where an unlisted terminal arrives as an ordinary unknown app and somebody allows one by mistake. §2.3's "reuse the ordinary approval path" yields to it **for this one approval**, because only a capture can reveal a shell in a window whose *app* no name list refuses. What §2.3 keeps is everything the ordinary path buys: the question is still a `RiskApprovalRequest` produced by the one requirement function, raised through the same `requestVisionActionApproval` every mid-loop action approval uses, so it renders on the floating widget *and* on `CommandCenterAttentionPanel` with neither surface taught anything about it. Consequences: `ApprovalContext.appControl` is `.notApplicable` at every plan-time construction site, the grant is minted by the person answering the in-session question and by nothing else, and the standing reaches no assessment at all.

**Per-app control note, added 2026-08-20 (row J, `feature/app-control-consent`) — read before trusting §11.2's account of what the approval engine takes as input.** The engine's posture input is no longer a boolean. `ApprovalContext.safeMode: Bool` is **replaced** by `ApprovalContext.mode: AgentInteractionMode` plus `ApprovalContext.appControl: AppControlStanding`, and `RiskApprovalPolicy.requirement(for:context:)` now switches over the whole tuple `(mode, appControl, effectiveTier)` with no `default:` and no wildcard in either authority position. Two booleans cannot express three modes, and the boolean fold made Normal and Power indistinguishable to the engine — correct while the engine distinguished two postures, and wrong the moment per-app control gave Power a rule of its own. §11.2 states the new inputs, the one-directionality rule the standing obeys, and the invariants the widening keeps (I8's one function; I4's tier-4 refusal in every cell; Safe's floor unchanged). §11.3 records that the standing changes *which* requirement is produced and never a sentence, so the per-app prompt reuses the ordinary approval path and therefore renders on both approval surfaces rather than needing one of its own. Durable decision record: `docs/sonny-row-j-plan.md` §2.5, and the founder decisions of 2026-08-16 on SONNY-104.

**Reconciliation note, added 2026-07-24 — read before trusting "menu-bar popover" or "dry run" anywhere below.** Two things this spec describes as current have since been superseded by actual implementation decisions, scattered across ~20 references each throughout this document that were not individually rewritten (the document is large enough that a blind find-replace risked getting some wrong — see the two clarifications below instead):
...
**Consequence-rule note, added 2026-08-13 (row C, `feature/approval-relaxation-structural`, superseding the relaxation note directly below — same day, same branch, before merge).** The founder issued a superseding product directive after the scope-conditional relaxation below was built and reviewed: **Sonny asks permission only when an action is destructive (destroys or replaces existing user data or user-built artifacts) or affects someone other than the user; everything else runs without asking, made legible by a ran-without-asking trace.** §11.2 now states the rule, the per-escalation consequence classification (destructive / affects-others / advisory, no default), the one-function discipline it kept, and the unattended ceiling's reachability argument; §11.3 states which surface each class reaches (panel or trace) and flags that `lightweightConfirmation` is no longer produced at all. Workspace scope verdicts survive as data — computed, stored, surfaced — and gate nothing. The relaxation grants described in the note below shipped, were ratified 2026-08-13, and were deleted the same day; the note is kept as the record of that arc. Durable decision record: `docs/sonny-founder-design-decisions.md` → "The consequence rule".

**Approval relaxation note, added 2026-08-13 (row C, `feature/approval-relaxation-structural`) — SUPERSEDED the same day by the consequence-rule note above; kept as the record of what was briefly true.** §11.2's "nothing relaxes on a verdict yet" is no longer true: being inside a workspace's scope now *reduces* prompting (in-scope tier 2 auto-runs, in-scope tier 3 drops to a lightweight confirmation), and a second, narrower grant covers plans the user built field by field on a UI surface. §11.2 states the two grants, the one-function rule, and the strengthened invariant (relaxation never writes `effectiveTier`, `escalations`, or `approvalCopy`); §11.3 records that relaxation changes the weight of an ask and never its sentence, plus the honest note that the two ask weights still render identically until row C's surface branch ships. The durable decision record is `docs/sonny-founder-design-decisions.md` → "Approval relaxation (roadmap row C)".
...
Prototype limitations to resolve before the major release, as the 2026-07-03 audit found them. Each is a finding of that date, not a statement about the tree today. **All seven have now been re-checked against the tree** — the storage one by SONNY-289 (2026-08-26), the other six by SONNY-296 (2026-08-27) — and every bullet carries its result inline: five are closed, one is closed on two of its three clauses, and one is open. A re-check note says what was verified and from what; it is never a claim that the closed ones were exercised in the running app, which is §4A.5's manual pass and `docs/sonny-manual-test-checklist.md`'s job. The full account, per finding, is the changelog's `docs/prototype-findings-rechecked` entry:

- Hacker News is a special-case web workflow; it should become a general web/source-to-Markdown capability. **Closed 2026-07-08 in `a09059c` and `edd4b8b`** — the first added public-page fetch with robots handling and readability extraction, the second the `web_to_markdown` operation with synthesis and Markdown save — and Hacker News stayed on as a preset source rather than the only one, which is what §4A.2 asks for. Search as an *input* to it got a real provider later, 2026-07-30 in `27015a0`. Re-checked 2026-08-27 against §4A.2's five acceptance criteria rather than against the files existing; each has a test asserting on written output.
- Music only opens provider results; it should attempt real playback through first-party provider APIs where available. **Re-checked 2026-08-27 and still open** — though no longer in the shape the audit found it. The capability is provider-aware now: `OpenMediaResultCapabilityAdapter` asks a playback provider first and opens a result only when that provider says no, and §4A.4's fixed failure-precedence order is implemented and tested. What does not exist is either provider API. `Sources/` and `Tests/` contain no MusicKit import, no `ApplicationMusicPlayer`, no Spotify Web API host and no PKCE (`git grep -lE 'import MusicKit|ApplicationMusicPlayer|api\.spotify\.com|accounts\.spotify\.com|code_challenge' 0985e5b -- Sources Tests` prints nothing and exits 1); the only two conformers of `SpotifyPlaybackProviding` and `AppleMusicPlaybackProviding` anywhere in `Sources/` are the `UnavailableSpotifyPlaybackProvider` and `UnavailableAppleMusicPlaybackProvider` stubs; and the shipping app passes neither into its executor, so both default to those stubs and **every `play_media` run takes the fallback-open route**. §4A.4's opener carries the same result.
- App opening is useful but shallow; it should become the foundation for app adapters and app-scoped actions. **Closed 2026-07-08 in `d3c5ce6`**, on the adapter foundation `2c74eae` had laid four days earlier: each of §4A.3's seven initial action types — the count is that section's own "Initial action types" list, read in this same file — has its own adapter declaring supported actions, required permissions, risk tier and fallback behaviour — six of them through the shared `LocalActionDescriptor` table (`git show 0985e5b:Sources/MacAgentCore/AppWebsiteActionDescriptors.swift | awk '/static let all:/,/^    \]/' | grep -cE '^        [a-z]'` → **6**), and `reveal_in_finder`, the seventh, inline in its own adapter. The flat launch allowlist the audit found is gone (C12, SONNY-82), while the search-URL allowlist §4A.3 asks to *keep* is still there. App-*scoped* action arrived with row J's per-app control standing, which reaches the approval engine as one of its two inputs rather than as a check chained after it (§11.2).
- The menu-bar popover is a strong cockpit, but account, settings, history, stats, privacy, and Power Mode controls need a full Mac app surface. **Closed 2026-07-10 in `e4c36f3`**, which added the Command Center window sharing one view model with the widget; the popover itself is gone rather than supplemented — `NSPopover` no longer occurs in `Sources/` at all (`git grep -l 'NSPopover' 0985e5b -- Sources` prints nothing and exits 1). All six named controls have a home: account on the bottom row, settings in `SettingsDialogView`'s five sections (`git show 0985e5b:Sources/MacAgent/CommandCenterView.swift | awk '/^private enum SettingsSection/,/^}/' | grep -cE '^    case '` → **5**), history on the Tasks page, stats on Insights and on Settings' Usage page, privacy on Settings' Data page, and the Safe | Normal | Power dial on Settings' Security & Access page. **What the tree cannot establish is that those pages render correctly** — this repository has no view-rendering tests and no view-inspection dependency, so the suite reaches the window, the destination list and the wiring, and nothing further; the rest is §4A.5's manual pass.
- Existing local tools are still too prototype-shaped; they need more generic schemas, reusable adapters, and capability-style naming before the hosted runtime work starts. **Re-checked 2026-08-27: two of the three clauses are closed and the third is open.** Reusable adapters and capability-style naming arrived with the adapter pass — 26 adapter files, each declaring a stable `local.*` capability ID, with every preview and execute call dispatching through `CapabilityRegistry` (`git ls-tree -r --name-only 0985e5b Sources/MacAgentCore | grep -c 'CapabilityAdapter\.swift$'` → 26). **Generic schemas did not.** A plan step is still one flat struct carrying a field for every operation, decoded against one key whitelist shared by every operation. Writing `S="git show 0985e5b:Sources/MacAgentCore/AgentPlan.swift"` for the shared prefix: `$S | awk '/^public struct AgentStep/,/^}/' | grep -cE '^    public var '` → **33** stored properties on `AgentStep`, of which `$S | awk '/^public struct AgentStep/,/^}/' | grep -cE '^    public var [A-Za-z]+: .+\?$'` → **30** are optional, against `$S | awk '/static let stepKeys: Set<String> = \[/,/^    \]/' | grep -cE '^        "'` → **30** keys in `AgentPlanDecoder`'s single global whitelist. So §4A.0's per-capability input/output schema, and its requirement that an adapter's input schema be validated the way plan steps are, are both unbuilt.
- Safety today is a single `dryRun` boolean, not the tiered risk/approval model described in §11. This needs to exist before Power Mode is built, not after (§11.1A, §21.0A). **Closed 2026-07-08 in `1b741cc`**, which wired tier-based approval gating through the runner and the UI on the tier/rule model `db2aec3` had added the day before; §11.2's consequence rule (2026-08-13) and row J's per-app input (2026-08-20) are later layers on that same engine, not replacements for it. No `dryRun: Bool` survives anywhere in `Sources/` (`git grep -lE 'dryRun *: *Bool' 0985e5b -- Sources` prints nothing and exits 1) — the `dryRunBehavior` strings that remain are capability metadata a planner reads, and `PriorTaskContext`'s `.dryRun` is a task-history outcome label.
...
Prototype behavior at the 2026-07-03 audit opened Apple Music or Spotify results but did not reliably play the requested track. **Re-checked 2026-08-27 (SONNY-296) and still open, with the halves separated.** The pre-major-release pass built the provider-aware capability this section asks for — `OpenMediaResultCapabilityAdapter` under §4A.0, the two resolvers below it, and the fixed failure-precedence order — and it is what the acceptance criteria at the end of this section are pinned against. What it did not build is either provider behind that seam, so nothing plays: §4's second prototype-limitation bullet carries the enumeration and the commands. **Whether playback is *reliable* is not a question this tree can answer**, because no wiring in the shipping app reaches a play path at all; the fallback the user actually gets is what `docs/sonny-manual-test-checklist.md` §7 has a row for.
...
- Permission center.
- Privacy controls.
- ~~Data Sent To AI history.~~ **Cancelled 2026-08-14 by founder decision (Sauransh Bhardwaj, on PR #49's two blocked surfaces; SONNY-88), reaffirmed for this surface 2026-08-21 (SONNY-17 section 3(d)) and reconciled here 2026-09-05 by SONNY-217.** The in-product ledger is deleted rather than deferred — no data-sent-to-AI mention exists anywhere in the product, the messaging story is website-era, and disclosure lives in exactly two places, Safe mode and the future website. Row 13's Command Center **Memory** section was built over the existing local stores and deliberately does not resurrect it. `docs/sonny-founder-design-decisions.md`'s "The transparency posture — ledger deleted, disclosure lives in Safe mode (2026-08-14)" section is the durable record. **This reconciles the mention in this list and nothing else**: §6.13, §14.4A and §14.5 are row 14's population, and the founders deferred §6.13 itself on 2026-09-04 (SONNY-106) as superseded by the 2026-08-14 no-explanatory-copy rule.
...
- Subscription state.
- ~~Trial state if offered.~~ **Superseded 2026-07-15 by §16.4's no-trial resolution, reaffirmed 2026-08-16 by the founder's pricing decision (SONNY-17) and retired here 2026-09-05 by SONNY-217** — there is no trial to hold state for. The permanent, non-expiring free tier is what a trial would have been, and row 13 shipped nothing that implements one: `grep -rniE '(^|[^A-Za-z])trial([^A-Za-z]|$)' Sources server/src | wc -l` answers **0** at `6cc9e189`, against a positive control of **209** for the same pattern with `subscription` in place of `trial` over the same paths. Widening the same pattern to both test trees (`... Sources Tests server/src server/test | wc -l`) answers **7**: five inside one Wikipedia HTML fixture, and two in `server/test/billing.test.ts`, where `trial` is a **made-up plan name** fed to the plan-map parser — which is the tier-count-is-configuration property seen from the other side, not a trial.
...
**The consequence rule (v1.2, founder directive 2026-08-13 — superseding, pre-merge, the same-day scope-conditional relaxation this section briefly described; the superseded text is preserved in §0's note and in `docs/sonny-founder-design-decisions.md`):**

**Sonny asks permission only when an action is destructive — it destroys or replaces existing user data or user-built artifacts (file overwrites, replace-on-save of routines/workspaces/snippets, deletions) — or affects someone other than the user (send/post/share/publish/purchase; no such capability exists in v1 today, so that class is armed but empty until vision actions arrive). Everything else runs without asking**, made legible by the ran-without-asking trace (§11.3).

Default rules under the consequence rule:

- Tiers 0–2: auto-run.
- Tier 3: explicit approval when any escalation is destructive or affects someone else; an advisory-only tier 3 auto-runs with its reasons on the trace; an escalation-free tier 3 (unreachable through any adapter today) fails closed to an explicit ask.
- Tier 4: refuse or require user takeover.
- Safe mode, evaluated first and unchanged: everything that could run asks explicitly — the cautious user's opt-back-in to being asked about everything.

Mechanically: every `CapabilityRiskEscalation` carries a **consequence class** — `destructive`, `affectsOthers`, or `advisory` — assigned at construction with no default, so a new escalation cannot land unclassified. This classification is also the fail-closed home for the genuinely irreversible future capabilities (§11.1's send, upload, delete, submit, purchase, share): when they arrive, their escalations classify into classes that already ask. `RiskApprovalPolicy.requirement(for:context:)` remains the **sole public path** to an approval requirement — the one-function rule stands, any escalation whose class asks first asks at *every* tier that can run (the rule, not the tier arithmetic, decides), and every authority axis lands as an *input* to this one function, never as a rule chained after its output.

**The inputs, as of row J (2026-08-20).** `ApprovalContext` carries two: `mode` (`safe` | `normal` | `power` — the user's posture dial, whole, replacing the `safeMode: Bool` that preceded it) and `appControl` (`notApplicable` | `allowed` | `needsApproval` — whether the run being judged is controlling an app the user has allowed Sonny to control, deliberately payload-free so the policy cannot be made to decide on an app's name). **`appControl` is resolved per iteration inside a vision session, not once per plan**, and every plan gate answers `.notApplicable` — a vision plan's included, per §0's ordering note; the sentence here said "resolved once per plan" and described only row J's first implementation. The switch is over the tuple `(mode, appControl, effectiveTier)` with **no `default:` and no wildcard in either authority position**, so a fourth mode or a fourth standing fails to compile until every one of its cells is answered rather than silently inherited. Three properties hold across the whole cross-product and are pinned cell by cell rather than sampled:

- **`appControl` is a strictness input only.** It may raise an ask and may never remove one — composed as `stricter(of:_:)` against a `.explicitApproval` floor, so the one-directionality is structural rather than remembered. Same rule screen-derived signals obey, and the same shape as C2's ratified "consent maps to a requirement override, never a tier change".
- **Tier 4 refuses in every mode and every standing** (I4), and Safe mode's floor is unchanged — Safe's answer is still `stricter(of: requirement(for: tier), safeModeFloor)`. Composing terms *inside* the one function is the ratified shape; what I8 forbids is a public function that takes a requirement and returns a different one.
- **The requirement function never writes the assessment, and never reads one it wrote.** It is `RiskApprovalPolicy.requirement(for:context:)` that this is a claim about: hand it one assessment and any mode or standing, and `effectiveTier`, `escalations` and `approvalCopy` come back untouched. The assessment itself is produced upstream by `AgentActionExecutor.assessRisk(plan:scope:)`, which takes **no** `ApprovalContext` — so `effectiveTier` is a pure function of the plan and the scope, which is what keeps the unattended path's fixed `.approved(.tier2)` ceiling honest, since that ceiling works by comparing tiers. (Stated this precisely after row J briefly forwarded one field of the context into `assessRisk`, making the looser phrasing — "byte-identical with and without any mode or standing" — false of the assessment while it stayed true of the function.)

The workspace scope verdict (§6.9) is **data, not a gate**: it is still computed and stored on the assessment — the task binding, the chips, and the future vision cage read it — and an out-of-scope resource still raises its tier-3 escalation with the same sentence (§11.3), classified advisory, so the fact reaches the user on the trace rather than a prompt. Being inside or outside a workspace's boundary neither reduces nor adds prompting. Advisory escalations still raise `effectiveTier`, so the tier stays an honest severity signal for every consumer that reads it.

The unattended ceiling is unchanged: a scheduled run still executes under a tier-2 standing grant, and every tier-3 it can actually reach is destructive — a scheduled run is unscoped (no out-of-scope advisory exists) and `edit_workspace` cannot be a routine step (no removal or widening advisory exists) — so it still asks, the standing grant still refuses it, and the schedule still pauses. Reachability pinned by a named test, not a type-level guarantee.
...
- Power Mode pauses per the consequence rule: destructive or affects-others actions ask; everything else runs with the trace.
...
The consequence rule and copy (v1.2, re-amended 2026-08-13). **The rule changes what asks, never what a sentence says.** Escalation sentences are computed from the assessment and reach their surface unedited; the consequence class decides only *which* surface:

- A destructive or affects-others escalation keeps today's approval panel — the same five lines above, the same escalation reasons.
- A run that executed without asking at tier 2 or above carries a one-line **ran-without-asking trace** on the widget's result panel. It names any advisory reasons verbatim — the out-of-scope fact, a workspace-entry removal, a whitelist-root widening: the sentences the approval panel used to carry, still read by the user — and states the rule itself when nothing was advisory ("nothing here is destructive, and it affects no one else"). Tier 0/1 never traces (that silence was always ordinary, and marking it would drown the signal); a run that prompted never traces (the prompt was the disclosure).
- **A per-app control question is an ordinary approval, asked at an unusual moment** (row J, 2026-08-20; ordering corrected 2026-08-21 — see §0). The standing changes *which requirement is produced* and never a sentence: the prompt is a `RiskApprovalRequest` like any other, carrying the app's display name as `involvedResource` and its escalation reasons, raised through the same call every mid-loop action approval uses, and answered by the existing Allow/Deny controls. That is what puts it on **both** approval surfaces — the floating widget and `CommandCenterAttentionPanel` — for free, where a bespoke panel state would have rendered in the widget only. What is *not* ordinary is when it is asked: after the session's first capture has cleared the terminal screen check, never at the plan gate, per `docs/sonny-row-j-plan.md` §4.3 (unnamed, and reading as this document's own §4.3, which does not exist, until SONNY-290 on 2026-08-26). The fact that allowing is remembered belongs in the escalation reason, the existing channel for *why the user is being asked*, and not in a new explanatory line: the product does not explain how it works.
- `lightweightConfirmation` is no longer produced by the mapping at all, so the lightweight-vs-explicit surface distinction this section previously required of branch 2 is moot unless a future rule reintroduces a light ask — flagged on SONNY-101 for the founder rather than silently dropped.
...
Status note (v1.2 observation, superseded 2026-07-09): the v1.2 audit (§4.1) recorded routines and workspaces as plain unencrypted JSON with zero Keychain calls anywhere, and asked for that to be corrected while productionizing the local storage layer (Workstream A). **It was true when written and stopped being true on 2026-07-09**, in `14402be`, which added `LocalStorageEncryption` — CryptoKit `AES.GCM` behind a `SONNYENC1` file header, keyed by a 256-bit key the Keychain holds under service `com.sonny.local-storage` — and retrofitted it into every local JSON store the repository had, `RoutineStore` and `WorkspaceStore` among them. Both take that key on their read and write doors (`AutomationStores.swift`, four call sites: `git grep -cE 'encryption\.(encode|decode)\(' 0df1f49 -- Sources/MacAgentCore/AutomationStores.swift` → 4). Every local store on disk now goes through that one shared pattern — thirteen of them at `0df1f49` (`git show 0df1f49:Sources/MacAgentCore/LocalStoreClassification.swift | sed -n '/^public enum LocalStore: CaseIterable/,/^$/p' | grep -c '^    case '` → 13). A file written before the retrofit carries no header, decodes once as legacy plaintext, and is rewritten encrypted on its next successful load, so "encrypted at rest" describes the steady state rather than every byte on every disk at every instant.

Keychain calls are not zero either. `KeychainSecretStore` is the only file in `Sources` that makes one, and it makes four — `SecItemCopyMatching`, `SecItemUpdate`, `SecItemAdd`, `SecItemDelete` (`git grep -coE 'SecItem(Add|Update|Delete|CopyMatching)\(' 0df1f49 -- Sources/MacAgentCore/KeychainSecretStore.swift` → 4, and the same pattern listing files rather than counting — `git grep -lE 'SecItem(Add|Update|Delete|CopyMatching)\(' 0df1f49 -- Sources` → that one path and no other). Count the bare token instead and the answer is 7 (`git grep -c SecItem 0df1f49 -- Sources/MacAgentCore/KeychainSecretStore.swift` → 7), three of those being `errSecItemNotFound` in status handling rather than a call — which is the figure the ticket that corrected this note carried, and it is a line count over a substring, not a count of Keychain calls.
...
- **Subscription plan — free plus exactly one paid tier** (founder decision 2026-08-16, Sauransh Bhardwaj, recorded on SONNY-17; the full shape, with what was declined, is §23's business-model bullet). A third tier is added when there is a real reason for it — team or Enterprise features, or an observed heavy-usage pattern — and not before. **Retired here 2026-09-05 by SONNY-217, after row 13 shipped**, and what shipped is not a fixed set of tiers at all: the plan list is one configuration value the gateway refuses to start without, of any length, so the tier count is data rather than code (`CREDIT_PLANS`, SONNY-212, PR #182 merged as `1dbbbb3f`). ~~Subscription plan (Pro and Max, see §23 for the resolved structure).~~
- **No trial (resolved 2026-07-15, replaces the earlier "trial if chosen"):** a permanent, non-expiring free tier instead — the mid-task-lapse principle below applies to a free user's credits running out the same way it applies to a paid lapse, which a time-boxed trial cannot honor (a countdown that cuts off mid-task is exactly the surprise this section exists to prevent). **Unchanged by the 2026-08-16 pricing decision, and confirmed against what shipped (2026-09-05, SONNY-217).** This bullet assumed no ladder — it names no tier — so the move to one paid tier leaves it exactly as written, and row 13 built the thing it describes: the free tier is permanent, its screen-control allowance is what runs out, and running out halts at the next step boundary rather than mid-action (SONNY-213, PR #190 merged as `15c7bd9f`). It is recorded here as confirmed rather than struck through, because a passage that still holds is not superseded by a decision that agreed with it.
- ~~Power Mode entitlement is gated to the Max tier specifically, not "paid" generally (resolved 2026-07-15) — Pro does not include it.~~ **Doubly superseded, and the two halves are different kinds of thing — SONNY-205 corrected one and deliberately left the other alone.** The *claim*, that Power Mode is entitlement-gated at all, is superseded 2026-08-16 by SONNY-17: screen control is the one paid feature line and Power Mode gates no feature of its own. The *framing*, a Pro/Max ladder, is superseded by the same decision — but §23's business-model bullet keeps that ladder deliberately, as history rather than as the plan, and the Pro/Max wording elsewhere in this section (the plan bullet above, auto-top-up below) is that same superseded framing. It is a different population from this sweep's two claims and is not this ticket's to rewrite. **That population was retired 2026-09-05 by SONNY-217**, row 13's records ticket: the plan bullet above and the auto-top-up bullet below now carry their own dated supersessions, and §23's ladder stays where SONNY-138 put it, underneath the current shape as history.
...
- Billing portal.
- Grace period handling. **Mid-task lapse behavior (clarified v1.2):** never interrupt a single atomic action in progress (e.g. between a click and its observation) — finish the current step, then block the next one with a clear message. Same graceful-halt shape as the permission-revocation path in §13.5, not a hard yank that could leave an app mid-form or mid-edit. **Auto top-up is opt-in and off by default** (founder decision 2026-08-21, Sauransh Bhardwaj, recorded on SONNY-17 section 1(c); shipped as SONNY-215, PR #196 merged as `2d03b0d5`), so it is *a* way a user meets this principle and not *the* mechanism that serves it — a user who has not asked for it meets the graceful halt above, which is what the principle actually rests on. Off by default is structural rather than a default value: an account with no consent row, or one whose `opted_in_at` is NULL, has not opted in, and the charge path reads that predicate (migration `0019_topping_up_happens_only_if_you_asked.sql`). A second founder decision, 2026-09-03 on the same ticket, adds what makes a standing card authorisation honest: the pack price on the switch itself, and the last charge's amount and date in Command Center's Account section — a price and a record, neither of which is explanatory copy under the 2026-08-14 rule. ~~**Auto top-up (resolved 2026-07-15)** is the mechanism that serves this principle for Pro/Max: a user running low mid-task tops up rather than hitting a wall.~~ **Superseded 2026-08-16/2026-08-21 (SONNY-17), retired here 2026-09-05 (SONNY-217)** — Pro/Max went with the ladder, and the claim that top-up is what serves the mid-task-lapse principle was true only of a user who has opted in.
...
**Migration is its own milestone (added v1.2):** the current prototype calls OpenAI directly from the client using an environment-variable API key (§4.1). Moving to this proxied model isn't an additive backend build — it's a full swap of the client's network layer, with device-session auth and entitlement checks threaded through the same path. What gets re-pointed is the six call sites that carry a provider credential today, enumerated at `fa22ef3` by reading every `session.data` call under `Sources/`: `OpenAIPlanner.swift:68` (`OPENAI_API_KEY`), `CerebrasPlanner.swift:81` (`CEREBRAS_API_KEY`), `OpenAITranscriber.swift:74` (`OPENAI_API_KEY`), `VisionModelClient.swift:191` (`OPENCODE_API_KEY`), `WebResearchSynthesizer.swift:345` (`OPENAI_API_KEY`) and `TavilySearchProvider.swift:65` (`TAVILY_API_KEY`). Three further files make requests without a provider credential and stay local by founder decision — `HackerNewsService.swift:46,55`, `MediaPlaybackService.swift:652` and `WebResearchService.swift:275,309` — so nine files hold eleven `session.data` calls between them. **`AgentViewModel` is not one of them.** It makes no network request at all: its only `URLSession`-adjacent lines are a doc comment at `:999-1000` and a `URLError(.cancelled)` comparison at `:1005`. It does change in row 12 — the readiness check, provider construction, and the duplicated error literals — but as the composition root, not as a client, and a session that reads this note as a work list will look for network code there and find none. (Until SONNY-138 this sentence named `OpenAIPlanner`, `OpenAITranscriber` and `AgentViewModel` as "every existing call site", which was wrong in both directions at once — it invented one and omitted four. Line numbers move; re-derive them with `git grep -n 'session\.data(' -- Sources` rather than trusting the ones above.) Treat this as a discrete, explicitly tracked deliverable in Workstream B (§21.2, §21.0A), not something that happens incidentally while building other backend pieces.
...
- Business model and subscription pricing (§16.4) — **the ladder below was superseded on 2026-08-16 by the founder's decision (Sauransh Bhardwaj), recorded in full on SONNY-17. Read the current shape first; the older ladder is kept underneath it as history, not as the plan.**
  ...
  **Current shape, 2026-08-16.** Free plus exactly one paid tier — chosen over keeping three tiers and over reserving a third for Enterprise, with a third to be added only when there is a real reason (team/Enterprise features, or an observed heavy-usage pattern). One feature line, and it is **screen control**; everything else — planning, opening apps, routines, workspaces, instant utilities, web research — stays free. Above that line **tiers differ by volume only, never by feature.** The free tier gets a small monthly allowance of screen control rather than a hard wall, so the free tier still demonstrates what makes Sonny different. Voice and instant utilities stay uncapped and free, for the unchanged 2026-07-15 reason that they are near-zero cost by design. Screen control gets its own credit weight. Metering and billing are built so the tier count is not baked in — that was part of the decision, not an embellishment.

  **What shipped, 2026-09-05 (SONNY-217, row 13's records).** The shape above stopped being a plan: row 13's billing half landed on six branches of its own, and every clause of it is now code. The paid line is screen control and it is the **only** capability in the product that blocks on billing — `Sources/MacAgentCore/ScreenControlGate.swift` is the one gate, consulted at two moments (session start, and each step boundary of a running session), and every other capability runs regardless of network, entitlement or allowance (SONNY-213, PR #190 merged as `15c7bd9f`). The tier count is a configuration value rather than a constant — `CREDIT_PLANS` has no default anywhere and the gateway refuses to start without it (SONNY-212, PR #182 merged as `1dbbbb3f`). The number a user sees is **screen-control runs left this month**; credits stay internal accounting, derived from metering rather than ledgered. Auto top-up is opt-in and off by default (SONNY-215, PR #196 merged as `2d03b0d5`). The subscription reaches the entitlement through a provider-agnostic seam, from a signed webhook (SONNY-211, PR #178 merged as `26adb350`), and the portal is the provider's own hosted one, linked from Command Center (SONNY-216, PR #183 merged as `60298cab`). **The payment provider is Polar** — founder decision of 2026-08-30 (Sauransh Bhardwaj and Bhavya), final rather than a shortlist entry, and it names which provider gets wired rather than retiring the seam: a merchant-of-record swap is a business decision that can recur.

  **Every dollar amount and credit count is deliberately unset**, and waits on two measurements that do not exist yet. The screen-control weight needs a measured per-session vision cost, which only row 12's metering can produce (screen control records no usage at all today). The web-research weight needs whatever provider-native search actually costs per call once SONNY-162 moves web search, page fetching and scraping off Tavily to OpenAI's API — a founder decision of 2026-08-17, scheduled for the last pre-release review round, which is after pricing would normally need to be settled. Both numbers were previously set on assumptions that turned out wrong, so neither is guessed here. **Still true on 2026-09-05, and one premise under it has moved (SONNY-217).** Every number is still unset and is still a release-time input; what changed is that the mechanisms that consume them all exist now, so setting them is configuration on a deployment rather than a code change. The web-research premise is superseded: the founders decided on **2026-09-04** (recorded on SONNY-106) that Tavily ships v1 and the move to provider-native search is revisited after release, so that weight no longer waits on a pre-release migration. The screen-control premise stands in substance and one parenthesis inside it went stale rather than moving: "screen control records no usage at all today" was measured on 2026-08-16 against `main` at `9cfa090` — the post-rewrite name of the `9a84e3b` SONNY-16's record cites — and stopped being true when row 12's metering landed. The gateway meters `screen.analyze`, and row 13's credit accounting reads exactly those rows and no others (`server/src/credit/store.ts`'s `PAID_ROUTE`). What is still missing is not the recording — it is a deployed gateway to record against, which SONNY-192 owes, and the founders' own pre-launch use on top of it. So the number is still unmeasured and is still not guessed here.

  **Why the 2026-07-15 ladder could not simply be carried forward, and the durable lesson.** The only non-volume thing its $100 tier offered was Power Mode — and Power Mode was, when this was written, identical to Normal in every observable way, so as recorded the top tier had no product reason to exist. (**Present tense corrected 2026-08-21**, PR #88's records round: row J gave Power a rule of its own — it is the one mode that asks about no app at all. The lesson below is unaffected and if anything sharper, since the feature's meaning changed underneath the tier a second time.) Meanwhile screen control, the actual differentiator, is free in all three modes because §6.5's paid-only entitlement gate was consciously parked (C11, a ratified timing deviation): it lands with or after row 13, and row 18 owns the gate itself while row 12 builds the entitlement check and the token it reads. The new shape claims that unclaimed line, and the volume-only rule above is the lesson: **a tier justified by a feature can be left justified by nothing when that feature's meaning changes underneath it.**

  **Superseded — the 2026-07-15 ladder, resolved during `feature/v1-strategy-replan`, which itself superseded an earlier "trial model" framing (a trial was considered and rejected, not just left undecided):** **Free** ($0/mo, 50 credits/mo, `gpt-5.4-mini`, no Power Mode) — **Pro** ($20/mo, 350 credits/mo, `gpt-5.5`, no Power Mode) — **Max** ($100/mo, 2,500 credits/mo, choice of `gpt-5.5` or Claude Sonnet 5, the only tier with Power Mode). Voice/dictation and instant utilities uncapped on every tier. 1 credit = one standard planner-routed command; web-research tasks cost 3 credits, reflecting Tavily's per-search cost (see §4A.2). Full credit-weighting rationale and cost model live in `docs/sonny-v1-implementation-changelog.md`'s branch 13 note. Its own caveat, which is the thread the 2026-08-16 decision pulled: Max's $100 was directionally set but not cost-confirmed until Power Mode's per-action cost was researched, and that research never happened.
