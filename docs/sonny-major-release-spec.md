...
- **The function still never writes the assessment.** `effectiveTier`, `escalations` and `approvalCopy` are byte-identical with and without any mode or standing, which is what keeps `effectiveTier` an honest severity signal for the unattended path's fixed `.approved(.tier2)` ceiling — a ceiling that works by comparing tiers.
...
- **A per-app control question is an ordinary approval** (row J, 2026-08-20). The standing changes *which requirement is produced* and never a sentence: the prompt is a `RiskApprovalRequest` like any other, carrying the assessment's own `involvedResource` (the app's display name) and its escalation reasons, and answered by the existing Allow/Deny controls. That is what puts it on **both** approval surfaces — the floating widget and `CommandCenterAttentionPanel` — for free, where a bespoke panel state would have rendered in the widget only. The fact that allowing is remembered belongs in the escalation reason, the existing channel for *why the user is being asked*, and not in a new explanatory line: the product does not explain how it works.
