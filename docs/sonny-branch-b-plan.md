...
|---|---|---|---|
| App | Raw user string, trimmed — `"Chrome"` and `"Google Chrome"` both persist as typed, and since SONNY-44 so do names outside the catalog (`CreateWorkspaceCapabilityAdapter.workspaceCreateSpec`) | Both sides through `WorkspaceScope.appKey`: `MacAppCatalog.resolve` to `bundleIdentifier` where it resolves, else `MacAppCatalog.normalize` of the raw name | Compare raw strings — the same app would match in one workspace and not another |
...
**The rule that decides which implicit apps count: an app the capability *drives* is a resource; an app the system *resolves* for a file already in scope is not.** Sonny AppleScript-controls Microsoft Word and Finder by name, so those are reported even though no plan field names them; `open_generated_artifact` hands its file to `NSWorkspace.shared.open` and Launch Services picks the handler by file type from the user's own defaults, so that app is *not* reported. The asymmetry is deliberate and load-bearing in both directions. Not reporting Word would be wrong: it is a specific app the capability chose and drives. Reporting the Launch Services handler would be wrong too, because nobody chose it — Sonny did not pick that app, the user's own file-type defaults did, and the file it opens was already scope-checked. **The conclusion stands; one of its original supporting arguments does not.**

*Amended 2026-08-05 (SONNY-44).* This paragraph originally justified the `open_generated_artifact` side with unremediability: *"`create_workspace` validates every app through `MacAppCatalog`'s twelve entries, so no user could ever add Preview or Quick Look to a workspace, and every 'produce a file and open it' plan would escalate forever with nothing to do about it."* **That argument is now dead in its own terms** — SONNY-44 decoupled scope listing from the launch catalog, so a user *can* now list Preview, Quick Look, or anything else. The rule survives on the choice argument alone, which is the one that was always doing the real work: a resource is an app the capability *drives*, and no amount of listability makes a handler the user's system picked into an app Sonny chose. The same now-dead argument is still written into `PlanScopedResources.swift`'s `open_generated_artifact` comment ("a handler no workspace could ever list"); that file was never-touch for SONNY-44, and correcting it is flagged on SONNY-37, which legitimately works there.

The related worry the original paragraph raised about Word — that it was reported as a resource while being unlistable — is simply resolved rather than amended: Word is listable now.
...
*Resolved 2026-08-05 (SONNY-44), the other way and better.* An in-scope-but-not-launched app **was** genuinely needed — Microsoft Word — and it did not need the additive field this paragraph forecast. Scope-only status is **derived** (the name does not resolve through `MacAppCatalog`), not stored: `apps` stays `[String]`, there is no schema change, and the one list keeps serving both jobs exactly as this section argues it should.
