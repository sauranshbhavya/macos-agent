...
**The rule that decides which implicit apps count: an app the capability *drives* is a resource; an app the system *resolves* for a file already in scope is not.** Sonny AppleScript-controls Microsoft Word and Finder by name, so those are reported even though no plan field names them; `open_generated_artifact` hands its file to `NSWorkspace.shared.open` and Launch Services picks the handler by file type from the user's own defaults, so that app is *not* reported. The asymmetry is deliberate and load-bearing in both directions. Reporting the handler would be unremediable — `create_workspace` validates every app through `MacAppCatalog`'s twelve entries (`CreateWorkspaceCapabilityAdapter.swift:87-89`), so no user could ever add Preview or Quick Look to a workspace, and every "produce a file and open it" plan would escalate forever with nothing to do about it. Not reporting Word would be worse: it is a specific app the capability chose and drives. That Word is *also* unlistable today is a real problem, but it is a problem with the catalog, not with the classification — which is exactly what SONNY-44 exists to resolve, and SONNY-44 must not resolve it by dropping the Word resource on the "unlistable" argument, because that argument belongs only to the system-resolved side of this rule.
