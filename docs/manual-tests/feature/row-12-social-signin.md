...
3. `com.sonny.macagent://auth/callback` into Authentication → URL Configuration → Redirect URLs,
   exactly.
4. The gateway's own variables for the real project exported in the launching shell (SONNY-280's resume
...
5. `./scripts/deploy.sh local`, then `defaults write com.sonny.MacAgent SonnyBackendBaseURL
