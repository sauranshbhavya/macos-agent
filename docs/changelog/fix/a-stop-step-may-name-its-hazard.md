...
- The gates every branch owes, run at `3a5af32f`, the commit that added this entry, and re-run at the head above it: `server/scripts/check-secrets.sh` → exit 0, `clean (1333 tracked files scanned, 12 patterns, 8 baselined fixtures)`; `scripts/no-attribution tree` → exit 0, `0 of 1329`; `scripts/no-attribution history` → exit 0 (its count is left out on purpose: it reads every ref in the object store the worktrees share, so it moved from 2583 to 2585 between two runs minutes apart while other lanes committed); `scripts/changelog-order` → exit 0, `26 entry file(s) under docs/changelog and 26 under docs/manual-tests resolve`.
