...
Status: complete — on `main` at `1832956e` with the full review's round and its follow-up pushed, holding for one reviewer pass covering both (it merges last in wave 12)
Date: 2026-09-18
...
- In a fifth round, every flow under a `product` record was judged on its own first step, and eBay's Watchlist flow was left out.
...
The figures, the gates, 22 of 22 mutants and all four rulings held up. Everything is resolved below, and SONNY-529 holds what that review routed elsewhere.

The coordinator's check of that round found F1's own defect surviving in one place. eBay's one `product` record stood for both of its flows, and the Watchlist flow's first step is the page that led to a bot check. That is resolved in round five below. One reviewer pass covers rounds four and five together.
...
Tests: the flagged `swift test` command from `CLAUDE.md` → exit 0, `Test run with 3475 tests in 251 suites passed after 107.147 seconds with 8 known issues` at `068c23a5`, clean tree. It started at 22:16:49Z with no Swift compiler or test process running anywhere on the machine and a one-minute load of 5.22, and no other worktree was building when it ended. Every one of the nine new tests prints one `started` and one `passed` line in that log; its 7 skip lines are environment gates, none Skill-related. `scripts/warnings` → exit 0, `measured at : 068c23a5 (clean)`, `0 warnings`. The Skills suites under `--filter 'SkillPack|Skill'` → exit 0, `Test run with 74 tests in 5 suites passed after 7.158 seconds`, over the tree committed as `068c23a5`. The server half did not move in rounds four and five, so its figures are carried from `38c7f731` with a tree-identity proof. `git rev-parse --verify --quiet "${rev}:server"` prints `98ab28c009c065c035d8a3b32efefcdccb71e403` at `38c7f731`, `a7bd61fe` and `068c23a5` (IDENTICAL). The same check reads `Sources` as MOVED, and a path that resolves nowhere exits 1 on both sides (REFUSED), which shows the check can give all three answers. The carried figures were owed by the hop's range (`git diff --name-only 0d645464 1832956e` → 105 files, 42 under `server/`), all run at `38c7f731`: `npm run build` exit 0; `npm run typecheck` exit 0; `npm test` exit 0, `Test Files 34 passed | 28 skipped (62)` and `Tests 951 passed | 503 skipped (1454)`, the database tests skipping without `DATABASE_URL`; and `npm run test:db` → exit 0, `Test Files 62 passed (62)` and `Tests 1454 passed (1454)`. That last one ran against a Postgres named `sonny-gw-db-lane-510` on the free host port Docker gave it (32778), after `pg_isready` and the init log's second "ready to accept connections"; the container was removed afterwards. The hop round's own Swift figures, at `38c7f731`, and round four's, at `a7bd61fe`, were each 3475 tests in 251 suites, 0 warnings and 74 Skills tests, and they are history now.
...
- **The full review's round: every `product` record judged again, on 2026-09-18.** First the profile was confirmed empty at 21:42:16Z: Google's identifier page with no account listed, and GitHub signed out. A `product` record has to mean the product is usable signed out at each flow's first step. So all 13 were read twice, signed out, through their own start URLs, and each flow's first step was looked for on the page. Where that step was gated, the record takes the sign-in rule: the site's own sign-in control was pressed and what opened was judged. Provider buttons were never pressed, and nothing was typed.
  ...
  - **Still `product`** (5 records across 4 packs):
    - **eBay**'s search box is its search flow's first step.
    ...
    - eBay's heading is the day's rotating promotion and is re-recorded as read. Its "My eBay" and `signin.ebay.com` both led to a bot check ("Please verify yourself to continue"), which a session never completes. So the Watchlist flow's first step could not be read, and round five leaves that flow out.
...
- **Round five: each flow judged on its own first step.** Round four asked whether each `product` record's word held, one record at a time. The question is per flow, because one record can stand for flows that are reached differently, and eBay's did.
  - **eBay's Watchlist flow is left out.** Its first step is "Go to My eBay and select Watching", the page behind the bot check above. A page that cannot be read cannot be verified, so the flow goes. What was tried: pressing "My eBay" on `www.ebay.com/`, and opening `signin.ebay.com`. Both led to the bot check, and nothing more was tried.
  - **The search flow keeps the record.** Its first step is the "Search for anything" box, which works signed out. The pack stays deep, with that flow, its record, its sign-in page and its triggers.
  - **The check, over every flow under a `product` record**, run from the repository root with the packs folder as its one argument. It prints the flow count, the flows under `product` records, and any `product` record covering more than one flow (arm 1). It then lists each flow under a `product` record whose first step names a place a signed-out visitor may not reach (arm 2), and each flow of any kind whose first step names account creation (arm 3):
    - `python3 -c "import json,glob,re,sys;from collections import Counter;P=[json.load(open(p)) for p in glob.glob(sys.argv[1]+'/'+'*.skillpack.json')];assert len(P)>100;G=re.compile(r'\bMy \w+|\bWatch(?:ing|list)\b|\bSaved\b|\bSettings\b|\bAccount\b|\bProfile\b|\bHistory\b|\bLibrary\b|\bDashboard\b|\bInbox\b|\bWorkspaces?\b|\bProjects?\b|\bSign in\b|\bLog in\b');N=re.compile(r'(?i)\bsign ?up\b|\bcreate (?:an |a |your )?(?:free )?account\b|\bregister\b|\bfree trial\b|\bget started\b');F=[(d['id'],f,{s['url']:s['offers'] for s in d.get('startPages',[])}[f['startURL']]) for d in P for f in d['flows']];C=Counter((d,f['startURL']) for d,f,o in F if o=='product');print(len(F),sum(o=='product' for _,_,o in F),[k for k,n in C.items() if n>1]);[print(o,d,'|',f['title'],'|',sorted({m.group(0) for m in G.finditer(f['steps'][0])} if o=='product' else set())+sorted({m.group(0) for m in N.finditer(f['steps'][0])})) for d,f,o in F if (o=='product' and G.search(f['steps'][0])) or N.search(f['steps'][0])]" Sources/MacAgent/Resources/SkillPacks`
    - **The control, first:** over a `git archive a7bd61fe` of the packs folder it prints `400 7 [('ebay', 'https://www.ebay.com/')]`. It names eBay's record, then `product ebay | View and tidy your Watchlist | ['My eBay', 'Watching']` and `product grok | Ask Grok a question | ['Settings', 'Sign in', 'Sign up']`.
    - **At `068c23a5`** it prints `399 6 []`, and then only the Grok line. No `product` record covers more than one flow now; before, eBay's was the only one.
    - **Grok's line is judged, not a finding.** Its first step names Settings, Sign in and Sign up only while describing the row across the top of the page. The step's action is the prompt box, which round four read signed out. The line is also arm 3's control: that pattern does find account-creation words in real step text, and it finds none in the first step of any of the 393 flows under a `sign-in` record.
    - Pointed at a folder with no packs in it, the command exits 1 rather than printing a zero. Its first draft did print a zero, over the archive's root instead of its packs folder.
  - **What the check cannot do.** It reads words, so a first step that names a signed-in place in other words passes it. That is why each of the 6 flows under a `product` record was also judged by reading it: eBay's search, Grok's question and Imagine, OpenRouter's catalogue, Perplexity's question and Zoom's join.
- **This branch removes packs, deliberately, and that is the rule working, not damage and not a regression.**
  - `main` at the hop, `1832956e`, has **189 deep packs and 473 flows**. With this branch it has **160 deep packs and 399 flows**: **29 packs and 74 flows removed**.
  - Every removed pack is one whose start page a flow may not start on, named above with its reason. The first round removed 10 packs and 24 flows, and the second removed 15 packs and 41 flows. The full review's round removed 5 packs and 11 flows and restored OneDrive's 1 pack and 3 flows. Round five removed 1 flow, eBay's Watchlist, and no pack. None of the 25 packs the hop brought in is removed, and eBay, one of them, keeps its search flow.
...
- **The guidance byte ceiling** is unaffected, because the records are not guidance. Every pack still loads under 6,000 bytes: `everyShippedPackLoads…` passes at `068c23a5`.
- **Against `main` at `1832956e`**, compared field by field over `git archive`s of both packs folders:
  ...
  - The other 189 are the deep ones. The 160 still deep carry `startPages`, and 29 went shallow. `signInURL` changed on 15, and `flows` changed on 60 (the 29 made shallow, Perplexity, eBay, OneDrive's moved start URL, and the start URLs the first round replaced). The same comparison at `a7bd61fe` counts 59, and eBay is the difference.
  - No flow's title, steps or source changed.
  - `docs/sonny-skill-sites.tsv` is as `main` has it: `git diff --quiet 1832956e 068c23a5 -- docs/sonny-skill-sites.tsv` exits 0.
...
- **The host rule names identity hosts in code, and the 15 `signInURL` corrections no longer matter to it.**
  - The first round corrected 15 packs' `signInURL` to the identity host their start page lands on. That put them at odds with their catalogue rows: `python3 -c "import json,glob,csv;R={r['id']:r['sign_in_url'] or None for r in csv.DictReader(open('docs/sonny-skill-sites.tsv'),delimiter='\t')};print(sum((json.load(open(p))['signInURL'] or None)!=R[json.load(open(p))['id']] for p in glob.glob('Sources/MacAgent/Resources/SkillPacks/'+'*.skillpack.json')))"` prints `0` over `1832956e`'s packs and catalogue, and `15` at `068c23a5`.
...
- **A record's word is judged per flow, not per record.** Round four's test was right and was asked of each record once. eBay's one record covered a flow whose first step works signed out and a flow whose first step does not, and the passing flow carried the other through. The population to judge is flows, each against the record its start URL names.
...
- **eBay's Watchlist flow is left out**, because a bot check stands in front of its first step. Restoring it means a reading of "My eBay" signed out that does not meet one.
- **The rule reads a flow's first step, and no step after it was judged.** A later step may still need an account. Whether saving a search on eBay does, for example, was not read.
...
Since the hop, `1aee8496` is the hop round's records for the 25 packs, `38c7f731` its refusal naming the missing pairing, `e7b7e944` its entry, `e3baddf6` the full review's code, `a7bd61fe` its data, `64040ab5` its entry, and `068c23a5` round five's eBay change. All seven are ancestors of this branch.

Every figure this entry states as current is measured at `068c23a5` or at `1832956e`, except the server half's, which is measured at `38c7f731` and carried by the tree-identity proof on the Tests line. `0d645464` is the branch's base and an ancestor of `main`.
