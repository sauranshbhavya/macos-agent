...
The work took two passes, a fix round and one hop onto `main`, all in one session. The coordinator's reply between the passes ruled on HubSpot and asked for Gong's third check. **SONNY-526**, one row only — `contactout`'s guessed sign-in address was opened and is right. Wave 13, cut from `main` at `8f3d1d02`, independent of every other lane — no stack. The branch took its one hop onto `main` at `7b89cfd9` on 2026-09-19, when the coordinator named it next.
Reviewed by: the coordinator measured HubSpot's start page independently between the two passes and ruled that it passes (SONNY-510). **review-281**, a fresh session with a browser connected, reviewed the pre-hop head `390e348e` (`949afef5` after the hop) and filed three blocking findings and two below the bar, all answered in the fix round:
...
- **F3 (Freshsales held on a spinner):** its record is corrected. The page renders once the window is drawn, so it is readable and not yet read. It is named for the next sales group by the coordinator's ruling, and SONNY-537 now carries it.
- **F4 (HoneyBook's third title):** taken in the fix round.
...
- review-281's delta pass over the fix round closed with nothing blocking. Its one note, a brief "Clay" title on one of two drawn arrivals, was added to Clay's `otherTitles` with the hop.

Spec sections covered: none directly. This is Skills pack data: fourteen resource files and fourteen rows of the catalogue.
Files changed, against the post-hop merge-base `7b89cfd9`:
- `Sources/MacAgent/Resources/SkillPacks/` `pipedrive`, `copper`, `nutshell`, `dubsado`, `kommo`, `insightly`, `clay`, `honeybook`, `contactout`, `rocketreach`, `hubspot`, `gong`, `outreach`, `linkedin_sales_navigator` (`.skillpack.json`):
  ...
  - Field by field against `7b89cfd9`, only `depth`, `flows` and `startPages` moved, on all fourteen and on no other pack. The command is a Python comparison of `git show 7b89cfd9:<file>` against `git show b3404dc7:<file>` for each of the fourteen. It prints `fields that moved on the 14: ['depth', 'flows', 'startPages']`, `flows 27 steps 120 start-page records 14` and `changed pack files == the 14: True`.
...
- `git diff --name-only 7b89cfd9 b3404dc7` lists those seventeen files and nothing else. No Swift source or test file is among them.

Tests: all at `b3404dc7`, the post-hop head's last pack commit, unless a line says otherwise. **Nothing is carried across the hop**: every figure below was re-measured there.
- **The hop.**
  - **The range:** `git log --first-parent 8f3d1d02..7b89cfd9` crosses three merges: PR #276 (`eed93521`), PR #278 (`30ef8c12`) and PR #277 (`7b89cfd9`). `git diff --name-only 8f3d1d02 7b89cfd9` is 42 files.
  - **What this branch uses in the range:** #278's fourteen other pack files and the catalogue, both of which this branch's completeness tests walk in full. None of the fourteen is one of this branch's packs (`comm -12` over the two branches' pack lists → 0). No Skill source or test moved: `git diff --name-only 8f3d1d02 7b89cfd9 -- Sources/MacAgentCore/SkillPack*.swift Tests/MacAgentCoreTests/SkillPackTests.swift Tests/MacAgentTestSupport/SkillPackFixtures.swift` → 0, against 1 for the control `Tests/MacAgentTestSupport/HangBackstop.swift`.
  - **The rebase:** `git rebase origin/main` replayed all eight commits with no conflict. The rebased tree `ad1fab99` equals `git merge-tree --write-tree 7b89cfd9 f301b764`, exit 0, so the hop changed nothing but the base. The same seventeen files differ from the base as before.
  - **Pre-hop SHAs are labelled history.** Every other SHA this entry names from before the hop (`63093311`, `4d2d49ad`, `a3e3af70`, `2cbfa1a3`, `b02d1cd6`, `390e348e`, `99d511ff`, `f301b764`) is a timestamp on the branch, and no figure below rests on one. Mapped by subject, which match one for one: `63093311`→`38917851`, `4d2d49ad`→`b93d4e05`, `a3e3af70`→`d5838b2a`, `2cbfa1a3`→`bd2bd2b3`, `b02d1cd6`→`fcef4dc0`, `390e348e`→`949afef5`, `99d511ff`→`81c5558e`, `f301b764`→`d4d85655`. On top of those, `b3404dc7` adds Clay's "Clay" title.
- **Deep and shallow counts.** `git grep -l '"depth": "deep"' <sha> -- Sources/MacAgent/Resources/SkillPacks | wc -l` gives 174 at `7b89cfd9` and 188 at `b3404dc7`. Of those 174, 160 were on the original base `8f3d1d02` and fourteen are #278's. The `"depth": "shallow"` twin gives 299 and 285. The control, `"depth": "medium"`, gives 0 at both. No catalogue row moved between `deep` and `site`, so the two tally numerals in `SkillPackTests.swift` are untouched and no Swift file changed.
- **The catalogue, cell by cell, re-baselined on the post-hop merge-base.** A scratch script compares `git show 7b89cfd9:docs/sonny-skill-sites.tsv` against the file at `b3404dc7`, cell by cell. It fails on any change outside the `doc_url_1`–`doc_url_3` cells of this group's 22 rows, or outside `contactout`'s `sign_in_url`, which SONNY-526 hands this lane. It also fails on any change to the header, the row set or order, a column count, or the trailing newline.
  - Result: `changed cells: 35 outside scope: 0`, exit 0. The 35 cells sit on exactly the fourteen deep rows, 2 or 3 each, and no `sign_in_url` cell is among them.
  ...
  - **Why the base matters:** the same file checked against the old base `8f3d1d02` reports `changed cells: 72 outside scope: 37` and exits 1. The 37 are the cells #278 moved on its fourteen rows (algolia to render), which are that branch's and not this one's.
- **The Skills suites under `--filter 'SkillPack|Skill'`**, at `b3404dc7`: exit 0, `Test run with 74 tests in 5 suites passed after 9.486 seconds.`
  - It started at 17:51:27Z with 0 Swift processes running (control `(zsh|launchd)` → 15) and a clean tree, at a one-minute load of 19.45. The load was 25.46 when it ended.
  - No line read `skipped`, and none read `recorded an issue` or `failed after`.
  - Two earlier runs are not cited. The first, at 17:48:01Z, also passed, but ten Swift processes from another lane were running when it started. The second, at 17:51:11Z, was refused before it started, because two were running.
- **The full flagged suite**: exit 0, `Test run with 3499 tests in 254 suites passed after 101.079 seconds with 16 known issues.` It started at 17:52:58Z on a clean tree at `b3404dc7`, with 0 Swift compiler or test processes on the machine (control `(zsh|launchd)` → 15), at a one-minute load of 14.72, and ended at 8.97. The process count came from `ps -axww -o command | grep -cE '(^| )/[^ ]*(swift-frontend|swift-driver|swiftpm-testing-helper|xctest)'`.
  - 0 lines read `failed after` or `recorded an issue at`.
  - The five catalogue completeness tests each printed exactly one `started` and one `passed` line (`grep -cE "Test <name>\(\) started"` and the same for `passed`, each → 1): `everyShippedPackLoadsAndEveryOneIsARowOfTheCommittedCatalogue`, `everyCatalogueRowHasExactlyOnePack`, `theCommittedCatalogueIsTheListTheFoundersDecided`, `everyShippedDeepPackRecordsWhereItsStartPagesLanded` and `everyShippedTriggerIsDistinctiveOrAnchoredToItsSite`. They walk all 473 packs and every catalogue row, including #278's fourteen.
  ...
  - The count moved from 3475 tests in 251 suites before the hop to 3499 in 254, and the known issues from 8 to 16. Both changes are the range's own tests (#276's and #277's), not this branch's.
  - **The pre-hop runs are labelled history.** Before the hop, the suite passed at `99d511ff` (`81c5558e` after the hop) at a load of 10.57. A run just before that failed under a load of 37 to 84, on main-actor-starvation backstops in `VisionSessionRunTests` and on the clock test PR #277 has since fixed, and was discarded. None of those figures describes this head.
- `npm run check:secrets` from the repository root, `scripts/no-attribution tree` and `scripts/changelog-order`: run over the tree committed with this entry, each exit read with nothing between it and `$?`.
  - `server/scripts/check-secrets.sh`: exit 0, `check-secrets: clean (1319 tracked files scanned, 12 patterns, 8 baselined fixtures)`.
  - `scripts/no-attribution tree`: exit 0, `0 of 1315 tracked file(s) carry an attribution (4 excluded by path)`.
  - `scripts/changelog-order`: exit 0, reading `main at 7b89cfd9`. After the hop the checkout contains all 21 branches merged in the one-file-per-branch era, #276 and #278 among them, and asks each for both files. It finds 22 entry files under each directory.
  - No Postgres was started and nothing under `server/` changed.
- **Ancestry.** every post-hop commit this entry cites exits 0 under `git merge-base --is-ancestor <sha> HEAD`, read with nothing between it and `$?`: `8f3d1d02`, `eed93521`, `30ef8c12`, `7b89cfd9`, the eight post-hop commits in the map, and `b3404dc7`. The eight pre-hop SHAs each still resolve here and exit 1, which is what makes them history rather than evidence. Among them the pre-hop head `f301b764` is the control, showing the check can fail. `ad1fab99` is a tree, not a commit.
...
- **Clay** — `app.clay.com/login`, titled "Clay | Login", with the h1 "Welcome back!". The first pass recorded the title "Go to market with unique data — and the ability to act on it". That is the page's static title, which an unpainted tab never replaces. Drawn, a sample every 250 ms from arrival read only "Clay | Login" (06:26:41Z), so the record's title changed. review-281's delta pass saw the page report "Clay" for a moment on one of two drawn arrivals before it settled, so `otherTitles` holds "Clay", the same shape as HoneyBook's record (added with the hop). "Sign up" links to `/signup`. Flows: build a table with Find Companies, and set up a Signal. The Find Companies page lists revenue brackets in dollars, and the flow leaves them out on purpose: a currency amount beside "Add" is a refusal under the money rule.
...
- The other 459 packs are untouched by this branch: `git diff --name-only 7b89cfd9 b3404dc7` names only the fourteen pack files, the catalogue and this branch's two record files. That includes the fourteen packs #278 made deep.
...
- The seven held packs and Freshsales are byte-identical to `7b89cfd9` (`git diff 7b89cfd9 b3404dc7 -- <file> | wc -l` → 0 for all eight, against 56 for the control, Clay), and their rows are unchanged.
