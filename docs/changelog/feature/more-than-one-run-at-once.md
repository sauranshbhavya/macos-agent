...
- `Sources/MacAgent/AgentViewModel.swift`:
  ...
  - The run's properties become computed properties forwarding to the slot of the run in scope: **45** (`git grep -c 'get { runSlotInScope\.' 2873d19b -- Sources/MacAgent/AgentViewModel.swift` → 45). Slots are written only through `updateRunSlotInScope`, which is `private`.
...
**Every figure here was measured at `2873d19b`, this branch's head after its second hop onto `main` at `ac0bfa2b`, with a clean tree.** This entry's last commit adds only record files, so `git diff --stat 2873d19b HEAD -- Sources Tests Package.swift` prints nothing, and each figure is the head's.

**Two hops, and why there are two.** `WORKFLOW.md` gives a branch one hop onto `main`, taken when it is next to merge. This branch took two, and the reason is a coordinator's error, not the lane's.

- **The first hop, which is history now.** It went onto `7b89cfd9`, after #276, #278 and #277 had merged. The coordinator had handed hop instructions to two branches at once while a third, #281, was still ahead of both in the order. Hops have to be sequential, because `main` moves at every merge. The instruction was withdrawn, but only after this branch had acted on it correctly and pushed.
  - The founders decided on 2026-09-19 to **leave that hop rather than rewind it**. A rewind needs a backwards force-push outside the standing authorization, the record edits re-applied as fresh commits, and a hop anyway: strictly more work, for a literal reading of a rule that exists to reduce work. The one-hop rule is an economy rule. It dates from a wave-9 lane that paid five hops with a full re-verification at each.
  - That hop's range was `git diff --name-only 8f3d1d02 7b89cfd9` → **42** files: 18 under `Sources/MacAgent`, fourteen of them skill packs, and 14 under `Tests/`. Two of them bore on this branch. `AppDelegate.swift` was edited by #276 in a region apart from this branch's. `HangBackstop.swift`, whose `waitOrAbandon` this branch's tests call, only gained a function.
  - At that hop's head everything was measured again, and every commit it measured has since been replaced by the second rebase, so no SHA is cited for it. The suite ran 3506 in 256 with 16 known issues, exit 0. `scripts/warnings` was 0. The ten mutants were proved again.
- **The second hop, the one that merges.** `git rebase --onto ac0bfa2b 7b89cfd9 feature/more-than-one-run-at-once` replayed this branch's ten commits with no conflict.
  - Its range is `git diff --name-only 7b89cfd9 origin/main` → **33** files, from #281 and #280. **28** are under `Sources/`, and every one is a skill pack (`grep -cE '^Sources/MacAgent/Resources/SkillPacks/[^/]+\.skillpack\.json$'` over that list → 28). The other **5** are records. Nothing is under `Tests/` or `server/`, no `.swift` file appears, and `Package.swift` is untouched.
  - That range is smaller than one hop from `8f3d1d02` would have been, which is why leaving the first hop was the cheaper path.
  - The branch uses none of it. But `SkillPackTests` walks every pack, so the suite and `scripts/warnings` were run again rather than carried. The ten mutants were proved again too, because every commit their earlier proofs cited was replaced.
- **Neither range adds a line that names anything this branch removed or reshaped.** Each range's `+` lines under `Sources` and `Tests` were searched with `grep -cE` for `\$(isRunning|approvalRequest|errorMessage)\b|postPermissionNotification|onAllow|markOutcomeAsNotified|approvalParked|errorMessageRaised|updateRunSlotInScope|runSlots|focusedRunID|RunScope|approvalToken`. That gives `git diff 8f3d1d02 7b89cfd9` → **0** and `git diff 7b89cfd9 ac0bfa2b` → **0**. The zero is controlled three ways:
  - the same pipeline over this branch's own `git diff ac0bfa2b 2873d19b` → 133;
...
**The flagged suite.** The command from `CLAUDE.md`, redirected to a file with `echo "SWIFT_TEST_EXIT=$?"` on the next line → **3506 tests in 256 suites passed after 100.736 seconds with 16 known issues, exit 0.**
...
- Load was `6.73` at the start and `6.50` at the end.
- **The count and the known issues held across the second hop, as they should when no test file moves.** The 16 known-issue lines name the same tests as at the first hop's head: the two sorted lists of test names `diff` with exit 0.
- They rose from 8 to 16 at the first hop. The extra eight are all in `CanaryBackstopTests.swift`, a file #277 added (3 + 2 + 3 lines across three of its tests), so none is this branch's.
- Every one of this branch's seven new or rewritten tests is named `passed` in the log, so none was skipped.

**Warnings: 0.** `scripts/warnings` exits 0, and its header reads `measured at : 2873d19b (clean)` and `compiled : every file`. It also started with no other Swift process running, at load `6.50` rising to `13.17`.

**Before either hop, kept as history and not as figures.** Every commit those runs measured has been replaced, so no SHA is cited.
...
- At round one's head, three full runs were recorded:
  - one red: all 25 failing tests were in MacAgentCore's `SonnyBackendClientTests`, timed out at load ~39, and that suite alone passed, 37 in 1;
  - one green but started beside another lane's Swift processes, so it does not count;
  - one green with the start condition met: 3482 in 253, exit 0.
- `scripts/warnings` was 0 each time it ran.
- `server/` is untouched, so none of its commands is owed.
Mutation plan: `mutation/plans/feature/more-than-one-run-at-once.txt` (founder-triggered, not run on this branch). **Ten** mutants. Each was applied by hand at `2873d19b`, after the second hop, built, and killed by the tests named here, then put back from `HEAD`. None of the earlier proofs was carried. The first hop moved R3's target file and a helper every killer calls, and the second replaced every commit the earlier proofs cited:
...
`scripts/mutate mutation/plans/feature/more-than-one-run-at-once.txt --check` → ten mutants, each `1 match`, at `2873d19b`. **Three more are owed by the next layer and are not in the plan.** They are listed under *Architectural decisions* below.
...
- **An error already set when Sonny launches no longer posts a "Task failed" notification (on purpose, founder decision 2026-09-19).** The old failure channel was `viewModel.$errorMessage`, and `@Published` sends its current value to every new subscriber. So an error set before `AppDelegate` subscribed was replayed into the notification. The new channel, `errorMessageRaised`, sends only errors that happen after it is subscribed.
  - **The known instance is the push-to-talk shortcut failing to register at launch.** `markVoiceHotKeyUnavailable` runs at `AppDelegate.swift:187`, before `observeNotificationTriggers()` at `:209`. Both lines come from `git grep -nE 'markVoiceHotKeyUnavailable|observeNotificationTriggers\(\)$' 2873d19b -- Sources/MacAgent/AppDelegate.swift`. The same command answered `176` and `198` before the first hop, because #276's edit to that file moved them. It answered `187` and `209` again after the second hop, which moved no Swift file. That was re-read rather than carried: a line number a command answered is a reading at a commit. The old build turned that into a "Task failed" banner whenever the user was not working in Sonny, though no task had failed. This build does not. **The failure is still visible in three places.** The widget shows the reason, because the error is persistent. Settings › Security & Access › Permission Readiness reads "Another app is using ⌃⌥Space." (`PermissionReadinessService.swift:190`). And the menu-bar icon turns to its failure colour.
...
- **`RunSlot.parkedApproval` has no reader.** `git grep -nE 'parkedApproval' 2873d19b -- Sources Tests` → the declaration alone, at `RunSlot.swift:161`. The next layer either uses it or removes it.
