...
- **Material**: a translucent "Liquid Glass" system, not a flat fill. **Corrected 2026-07-16: the widget and the notification banner do not share one recipe — verified directly against the raw wireframe SVGs/CSS, not assumed from the shared "System B" framing.** Floating widget panels: base `#1A1A1A`, 3 layers (`#1A1A1A` opaque + `rgba(26,26,26,.5)` twice), blend modes `lighten, luminosity, luminosity`. System notification banner: a **different** base `#333333`, only 2 layers (`rgba(51,51,51,.1)` + `rgba(51,51,51,.3)`), blend modes `normal, luminosity` — no `lighten` pass, no doubled layer. Do not build one shared "glass fill" modifier parameterized only by radius/shadow-offset — the notification needs its own fill recipe, not a retint of the widget's. Edge/rim (shared, confirmed identical on both) is not a real border — it's a 3-pass hairline `#A6A6A6` box-shadow (`±1.25px` offset + a `0.5px` outline pass, blend `plus-darker`).
- **Panel radius**: **not uniform across System B — two distinct values.** The floating widget's own pill and expanded step-log panel use `34px` (on the 40px-tall pill this exceeds half-height and renders as a full stadium; on the taller panel it renders as a genuine soft rounded rect). The **system notification banner uses a different, smaller radius: `20px`** on its 344×56 card — don't apply the widget's 34px to notifications, they're visually distinct components that happen to share the same shadow/glass recipe (§3.2). Circular buttons in both: `1000px`. **Note for implementers (added 2026-07-16):** the raw pill SVG's own `rx` attribute reads `20`, not `34` — this is the exporter baking in the geometric clamp (radius can't exceed half the 40px height) at render time; the *authored* Figma value, confirmed via the paired CSS export, is genuinely 34px. Don't trust the raw SVG's `rx` alone for this one element — check the CSS export too. SwiftUI does not necessarily auto-clamp an oversized `cornerRadius` the same forgiving way CSS does, so prefer `Capsule()` over a hardcoded `cornerRadius: 34` on a fixed 40pt-tall bar.
- **Accent colors — do not reuse System A's `#5C84FE`.** These are per-action, not one universal accent:
  ...
  - `#FF383C` — **corrected 2026-07-16**: this is the task-level-failure **retry** button specifically (a 23×23 circular button attached to the "Sonny failed to complete the task" row in `8-FloatingWidgetFailure.svg`, icon path confirmed identical to the step-level retry icon on `7-FloatingWidgetError.svg`), not a dismiss/error button — there is no dismiss button anywhere in the System B wireframe set. Distinct from the softer `#FF7474` icon color used for a failed row's own icon glyph. See the corrected §3.3.6 below.
...
Inner glass highlight — **corrected 2026-07-16: blur differs between the two surfaces, verified against the literal CSS export on both sides, not a rendering artifact.** Floating widget: `inset 0 40px 10px -40px #1A1A1A, inset 0 -40px 10px -40px #1A1A1A`. System notification banner: the same two-pass structure but **half the blur** — `inset 0 40px 5px -40px #1A1A1A, inset 0 -40px 5px -40px #1A1A1A`. Tinted circular buttons only get one more highlight layer on top: `inset 0 40px 30px -40px #E6E6E6`.
...
6. **Task-level failure (terminal)** — after the step rows, a final line reads "Sonny failed to complete the task" in full white (confirmed verbatim from the SVG's literal text content, high confidence). **Corrected 2026-07-16 — the previous draft misidentified the retry control:** the retry button is a separate 23×23 circular button attached directly to this failure-text row (`fill="#FF383C"`, icon path confirmed byte-identical to the step-level retry icon in state 5), not the 36×36 orange pill-adjacent button. That orange button is confirmed pixel-identical to idle's own mic button — same glyph codepoint, same position, same tint — and is unchanged in the *success* state too, where "retry" would be meaningless; it's carried over from idle, not repurposed. So this state offers both: type a new command via the (unchanged) pill, or retry the whole failed task via the small red button on the failure-text row itself — it is not a hard dead-end requiring a fresh command. This is still a distinct resolution path from step-level error (§3.3.5's inline per-row retry) — one retries a single step, this retries the whole task.
