...
- [ ] **(new 2026-09-13, SONNY-465) A look for the next pass, not a check.** The founders kept the Tasks page as shipped and asked to look at two things on it (2026-09-13); this is the first. Type a word no task contains into "Search tasks". The list gives way to the no-results state: the icon, "No matching tasks", and "Try a different word, or clear the search.", with no button under it. Look at how it reads, and whether the way back out is obvious: the clear control in the field and Escape both empty the search. Nothing here changed, and what the search does is covered by the rows above, so a state that looks the same as before is not a regression to report.
- [ ] **(new 2026-09-13, SONNY-465) A look for the next pass, not a check — the two delete confirmations.** Open a finished task that ran a screen-control session, so its receipt shows "What Sonny did on screen". The receipt's … menu › **Delete task** asks "Delete “`<the command>`”?", with "This also deletes what Sonny did on screen for this task." under it, and offers **Delete task** and **Cancel**. The … menu on the "What Sonny did on screen" section › **Delete what Sonny did on screen** asks "Delete what Sonny did on screen?", with "This deletes the screenshots from this Mac and from Sonny's servers. The task stays in your history." under it, and offers **Delete screen record** and **Cancel**. Look at the wording of both, then press **Cancel** on each: the task and its screen record are both still there. A task with no screen record asks the first question with no second sentence and has no second delete. What each delete removes is checked in "Delete a task and the server's copy goes with it" and "The other three delete buttons reach the server too"; this row only looks, so wording that reads the same as before is not a regression to report.
...
- [ ] **(new 2026-09-13, SONNY-465) A look at the Account section as a design, signed in — not a check.** The founders kept it as shipped and asked to look at it in the next pass (2026-09-13). Sign in to an account that has a subscription, screen-control runs and a past top-up (the setup is in "Is this user allowed, and have they used more than they are allowed" and "Buying more runs when they run out"), then open the account row's menu › Account. Under your address and **Sign out** it reads, in this order: the plan line (`<Plan> · Active`) with **Manage subscription** beside it; "Screen Control" with "`<n>` of `<m>` runs left this month"; the switch "Buy more runs when these run out (`<price>`)"; and "Last top-up" with "`<amount>` on `<day> <month>`". Look at the section as a whole: spacing, alignment, and how the rows read together. A row with nothing to show is absent rather than empty (no plan line without a subscription, no switch when top-ups are not sold, no last top-up before a charge), and a failed payment reads "Past due" with **Update payment**. What each row does is checked in those sections, so a row that looks the same as before is not a regression to report.
...
founders' pass, test 20). It names the items now, by the name Finder shows: one item by name; more
than one behind their total, up to five named while the sentence fits the widget's three lines, and
the rest counted.
...
- [ ] Select three short-named files: "3 selected in Finder: a, b, c." with all three named, in
...
- [ ] Open **Skills** and scroll the whole list, top to bottom. It scrolls without stutter or blank rows,
      ...
      you added it earlier). **What would be a finding:** a row with no description, or a description
      that talks about Sonny rather than the site.
- [ ] Search. `stripe` leaves only **Stripe**, reading "Online payments infrastructure.". `slack` leaves
      only **Slack**. `hubspot.com` leaves only **HubSpot**. `gmail` leaves **Gmail** and **Streak**
      (Streak's line is "CRM inside Gmail."). `zoho` leaves eight: Bigin by Zoho CRM, Zoho Books, Zoho
      CRM, Zoho Calendar, Zoho Creator, Zoho Desk, Zoho Forms and Zoho Mail. The list filters as you
      type, with no pause you can see. Clear the search.
- [ ] **Add Gmail and Calendly.** Each row gains the **Added** badge and its button reads **Remove**.
      Quit Sonny (⌘Q), relaunch, open Skills: both are still added. **Remove Calendly**: its row reads
      **Add** again and Gmail's still reads **Remove**. Remove Gmail too, so the next pass starts clean.
...
- [ ] On the Skills page, **add Bitly**. Ask the widget: `shorten this link in bitly
      https://www.anthropic.com`. Deny at the first approval. On the Tasks page, that task's plan names
      `https://app.bitly.com/` as where it starts and follows the pack's steps: **Create new** in the left
      sidebar, then **Shorten a link**. **What would be a finding:** a plan that searches the web for
      Bitly, names no Bitly URL, or goes through a page the pack does not mention.
- [ ] **Add YouTube Studio.** Ask the widget: `upload a video to youtube studio`. Deny at the first
      approval. The plan names `https://studio.youtube.com/` as where it starts and goes through
      **CREATE**, then **Upload videos**. Then **remove Bitly and YouTube Studio**, ask the Bitly command
      again, and deny again: this time the plan is Sonny's ordinary one, with nothing from the pack.
      **What would be a finding:** the first plan not using the pack, or the second one still using it.
