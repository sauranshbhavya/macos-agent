...
also match on a phrase with the site's name in it, such as `close crm`, because their name alone is
an ordinary word. Signed in, gateway up. These rows only read the plan: stop the task before it acts
if you do not want the change made in your own account.
...
- [ ] **Add Close.** Ask: `log a call with Acme in close crm`. The plan names `https://app.close.com/`
      as where it starts. Then ask: `close the finder window`. That plan names no Close URL.
      **What would be a finding:** a Close URL in the second plan, or none in the first.
- [ ] Remove Mailchimp and Close, so the next pass starts clean.
