...
transaction. Server half only: nothing in the app changes, and nothing on the Mac reads the count.
...
- [ ] The cancel tests, at the lane database:
      ...
      The runner names, and passes, four tests: `DELETE /v1/account/content cancelled between the
      ...
      pass records every body`, and `DELETE /v1/account/content?before bounds the stored bodies it
      clears and records by claimed_at`. The tally reads `Tests  4 passed (4)`. **What would be a
      finding:** any of the four failing, or the tally reading `skipped` — a skipped database test
      reads like a passing one, and it means `DATABASE_URL` did not reach the run, so nothing was
      tested.
- [ ] `npm run test:db` at the lane database: the whole database suite passes with those four.
