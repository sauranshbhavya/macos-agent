...
- The three non-workspace browser callers are untouched and still use the system default: `OpenSafeURLCapabilityAdapter` (a plain "open github.com"), `OpenAppSearchURLCapabilityAdapter`, and `WebResearchMarkdownCapabilityAdapter`'s Hacker News link. They call the `open(_:)` shorthand, which forwards to `open(_:using: nil)`.
