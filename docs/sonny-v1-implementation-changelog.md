...
Spec sections covered: §10/§11 (risk and approval) audited, not revised — no tier definition, no requirement mapping, and no policy default changed. §3.3.3's permission panel gains first-run-only copy; its steady state is untouched.
Files changed:
- `Sources/MacAgentCore/AgentActionExecutor.swift`
- `Sources/MacAgent/AgentActivityPresentation.swift`
...
- **The steady-state permission panel is byte-identical to the wireframe.** `firstRunApprovalExplainerLines` returns an empty array once `hasCompletedFirstApproval` has flipped, pinned by `noExplainerLinesOnceTheFirstApprovalHasBeenResolved`.
- **`hasCompletedFirstApproval` itself is untouched** — still `private(set)`, still flipped at exactly the two real resolution points (`performApproval`'s allow branch, `cancelCurrentRun`'s deny branch), still read with `object(forKey:) as? Bool`.
