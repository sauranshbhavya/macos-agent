...
Reviewed by: fresh session (per WORKFLOW.md step 7) — pending at time of writing; this entry is written before the PR opens, per WORKFLOW.md step 7.
...
- `Tests/MacAgentCoreTests/RoutineActivationTests.swift` (new, 11 tests)
...
Tests: `env CLANG_MODULE_CACHE_PATH="$PWD/.build/clang-module-cache" swift test --disable-sandbox -Xswiftc -F -Xswiftc /Library/Developer/CommandLineTools/Library/Developer/Frameworks -Xlinker -rpath -Xlinker /Library/Developer/CommandLineTools/Library/Developer/Frameworks -Xlinker -rpath -Xlinker /Library/Developer/CommandLineTools/Library/Developer/usr/lib` -> exit 0, **493 tests in 47 suites** at `d329ea9`, up from the **471 in 45** re-measured on the branch base `ca4fbe4` (= `origin/main`). Intermediate: 480 in 45 at `7727a58` (SONNY-52). `swift build` -> exit 0 at every point. Every count above carries the SHA it was measured at, per the convention SONNY-24's entry introduced.
