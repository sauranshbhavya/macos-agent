...
| B | `feature/workspace-restriction-scope` | §6.9, §10.4, §11.2, §11.3, §9.2, §9.3 (amended by SONNY-42) | **Planned 2026-08-04, not started.** SONNY-12 ran steps 1–2: founder decisions taken with the co-founder, the recorded interaction flag resolved, and seven implementation tickets created and user-approved (SONNY-36 → SONNY-42, serial, one branch). Full plan: `docs/sonny-branch-b-plan.md`; durable decisions: `docs/sonny-founder-design-decisions.md` → "Workspace restriction scope (roadmap row B)". **Hard sequencing constraint: SONNY-37 onward cannot start until SONNY-29 merges** — it rewrites the same `AgentActionExecutor.assessRisk` that a scope check must hook into, and today's unsegmented walk would make a scope check see only the first step of a repeated operation. SONNY-36 (pure model/evaluator) is unblocked now |
