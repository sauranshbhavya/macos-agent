...
- **Positioning copy.** A sharpened direction was recommended (lead with "one continuous trust model, from trivial to serious" in first-run/onboarding copy specifically, not as a headline-tagline replacement) but not confirmed — still genuinely open.
...
- **Branch 17 (`feature/mcp-client-integration`)** is new, added 2026-07-15 during `feature/v1-strategy-replan`'s Phase 4 — Raycast and other competitors now ship MCP client support, and Sonny has no equivalent. Locked now, non-negotiable: every MCP tool call is a `CapabilityAdapter` like any other capability, routing through `AgentRunner`'s existing risk-tier/approval gate with no parallel or bypassing trust path — this is the rule the whole product's trust story depends on and it must not be relitigated at implementation time. Explicitly left open for build time: which MCP servers/tools ship first, server-configuration UI, and initial risk-tier defaults for MCP tools pending real usage data — that ecosystem moves too fast to lock specifics now. Sequenced immediately before Power Mode; this placement is confirmed. **The rationale for that exact placement is provisional, not settled** — one reading (MCP as a lower-risk dry run for handling not-fully-controlled external capabilities, with Power Mode inheriting maturity from that experience) is coherent but has not been explicitly confirmed by the user. Do not scope additional branch-17 requirements around "stress-testing approval-flow edge cases for Power Mode's benefit" until that reading is confirmed.
