...
| C | `feature/approval-relaxation-structural` | §10/§11 revisions, scoped in its planning phase | Not started — the structural half of approval relaxation, justified by restriction scoping existing (branch B first). **Two hard constraints already handed to it by SONNY-12's planning, recorded in `docs/sonny-founder-design-decisions.md`:** relaxation is a *requirement* override and must never lower `effectiveTier` (four things depend on that field staying honest, including the unattended gate); and the in-scope tier-2 and tier-3 rules are one function of `(tier, verdict)`, never two chained rules — chaining them would let in-scope tier-3 actions silently auto-run |
