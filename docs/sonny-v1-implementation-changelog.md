...
Tests: `env CLANG_MODULE_CACHE_PATH="$PWD/.build/clang-module-cache" swift test --disable-sandbox -Xswiftc -F -Xswiftc /Library/Developer/CommandLineTools/Library/Developer/Frameworks -Xlinker -rpath -Xlinker /Library/Developer/CommandLineTools/Library/Developer/Frameworks -Xlinker -rpath -Xlinker /Library/Developer/CommandLineTools/Library/Developer/usr/lib` -> exit 0, **463 tests in 45 suites**, up from the 448 in 45 re-measured on `main` at `615d6b0` (SONNY-29 and the row-B planning PR both merged). `swift build` -> exit 0.
...
- **A schedule edit no longer loses a pause** (PR #27, F1). `commitScheduleDraft` carries `pausedReason` through `newlyCreated`, which sets it before calling `setEnabled` — so the single rule about clearing a pause stays in one place: enabling clears it, staying disabled keeps it. Pinned in both directions (`editingAPausedRoutinesRunTimeKeepsItPausedWithItsReason`, `editingAPausedRoutineWhileSwitchingItBackOnClearsThePause`) so the carry-over cannot degrade into a pause that can never be cleared.
...
**PR #27's review corrected this entry's original causal claim, and the correction is worth keeping visible** because a future session reading only the changelog would otherwise inherit a false model of the scheduler. The first version of this paragraph said a reverted `lastRunAt` would put the occurrence back in play and resurrect the forever-retry. That is empirically false: the reviewer implemented the caller-held write-back and the full suite still passed 463/463, because `RoutineScheduler.decision` returns `.notDue` for any disabled schedule (`RoutineScheduler.swift:46-48`) and every re-enable re-anchors the baseline anyway. A paused schedule cannot retry regardless of what its baseline says. The store-boundary rule above is the real, and stronger, justification — it is a documented invariant of the type rather than a downstream symptom.
...
- **Accepted unchanged (PR #27, F6): the `pausedReason != nil` ⇒ `!isEnabled` invariant is documentation-enforced, not type-enforced.** `pause(reason:)` is the only writer and it always disables, `setEnabled(true,)` always clears, and the field's own doc comment states the implication — but nothing stops a future caller assigning `pausedReason` directly on an enabled schedule, which would render a "Paused" caption on a routine that is merrily still running. Making it structural (a private setter, or folding both fields into one enum) is a public-API change to a `Codable` type with a persisted representation, which is more than this ticket's scope and would need its own migration story. Left as is deliberately, and recorded here for whoever next touches the type.
