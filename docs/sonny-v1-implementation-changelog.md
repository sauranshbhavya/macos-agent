...
Tests: **not run, deliberately** — the diff is confined to two root Markdown files and two files under `docs/`, which is exactly the case step 7 exempts, and no `Sources/` or `Tests/` file exists in it (`git diff --stat ca4fbe4..cea2382` lists four `.md` paths and nothing else). The suite's last measurement stands unchanged and is quoted rather than re-asserted: 471 tests in 45 suites, the figure the SONNY-24 entry above records for `feature/sonny-24-routine-browser-binding`, merged as `ca4fbe4`. This branch adds no code for it to move, and it was not re-measured here. Verification here is the read-through pass, a key-phrase sweep, and the adversarial audit above — all recorded on SONNY-57's closing comment with their evidence.
