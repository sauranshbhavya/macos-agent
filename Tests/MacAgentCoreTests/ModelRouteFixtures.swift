...
struct RecordedBackendRequest: @unchecked Sendable {
    let path: String
    let method: String?
...
final class RecordedBackendRequests: @unchecked Sendable {
    ...
    var all: [RecordedBackendRequest] {
        lock.lock()
        defer { lock.unlock() }
        return recorded
    }

    var only: RecordedBackendRequest {
