...
- interact_with_app: Make a new note in the Notes app
  description: Start a new note in the Notes app, in whichever folder is open there, and put the user's text in it. Use this only when the user names the Notes app: "make a note in Notes", "add a note to Notes". A note request that does not name the Notes app stays with create_local_draft, and every other app stays with its own operation. Use it as the plan's only step. Set appName to Notes, interactionGoal to the outcome in one sentence, interactionTarget to null, and interactionText to the note's text word for word; it may run over several lines.
  required fields: appName, interactionGoal, interactionText
  side effects: Starts a new note in the folder open in Notes and puts the text in it, Sends the labels of the Notes window's controls to Sonny's model, which stores none of it; never the text of any note
  dry run: Describe the app and the note's text; touch nothing.
  examples: make a note in Notes saying buy milk and eggs | add a note to Notes: call the dentist on Monday
