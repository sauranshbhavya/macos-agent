...
struct PlannerBoundaryTests {
    ...
    func defaultSystemPromptMatchesGolden() {
        ...
        - For "draft a WhatsApp to Mom saying I'll be late", "write 'see you at 6' to Alex on WhatsApp but don't send it", or any request to leave a message unsent in WhatsApp, produce exactly one interact_with_app step and no other step, with appName WhatsApp, interactionGoal holding the outcome in one sentence, interactionTarget holding the chat or contact name exactly as the user said it, and interactionText holding the message word for word. WhatsApp only: a draft in any other app, and any request to send, stays with vision_session.
    ...
    func theAgentPlanSchemaKeepsItsStrictShape() throws {
        ...
        #expect(schema["required"] as? [String] == ["summary", "requiresConfirmation", "steps", "itemJob"])

        // SONNY-235. Named by value rather than checked for presence, because the two things that
        // must not drift are what a model may say inside a job and what it may not: `items` is
        // absent here and from `AgentPlanDecoder.itemJobKeys`, so the forty paths a job acts on are
        // read from the machine and never asserted by a model.
        let itemJob = try #require((schema["properties"] as? [String: Any])?["itemJob"] as? [String: Any])
        #expect(itemJob["type"] as? String == "object")
        #expect(itemJob["additionalProperties"] as? Bool == false)
        #expect(itemJob["required"] as? [String] == [
            "source",
            "folderPath",
            "itemKind",
            "fileExtensions",
        ...
        #expect((itemJob["properties"] as? [String: Any])?["items"] == nil)
    ...
    func theBrowserDefaultIsStatedWithoutWeakeningTheGeneralClarifyRule() {
        ...
        // Direction 3 — a named browser is carried rather than dropped.
        //
        // **This assertion changed in SONNY-157 and the change is the point.** When SONNY-152 wrote
        // it, naming a browser had no routing effect anywhere: `AgentStep` had no browser field and
        // the only non-nil `preferredBrowser` in `Sources/` was the routine path. So the rule
        // deliberately promised nothing — only that the page still opened rather than becoming a
        // clarify or an unsupported step — precisely so the prompt could not claim routing the
        // system could not do. SONNY-157 built the routing, so the prompt now says the true thing.
        // The promise got stronger; it was never wrong.
        #expect(prompt.contains("set browserName on that step to the name they used and produce the URL step as normal"))
...
- interact_with_app: Draft in WhatsApp without sending
  description: Open a chat by name in WhatsApp and leave text there, unsent. Use this, and not vision_session, when the user asks to draft, write or prepare a WhatsApp message without sending it. WhatsApp only for now: a draft in any other app, and any request to send, stays with vision_session as before. Use it as the plan's only step. Set appName to WhatsApp, interactionGoal to the outcome in one sentence, interactionTarget to the chat or contact name exactly as the user said it, and interactionText to the exact text to leave.
