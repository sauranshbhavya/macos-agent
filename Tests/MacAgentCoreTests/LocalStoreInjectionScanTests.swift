...
///
/// All six are subsumed because `fileURL` is now a required parameter of every store
/// initializer and `RoutineStore()` does not compile. A scan that re-proves what will not build is
...
/// case, so a new store cannot reach the tree without being mapped here — and being mapped
...
struct LocalStoreInjectionScanTests {
    ...
    ///
    /// A `switch` with no `default`, so a new `LocalStore` case does not compile until
    ...
    static func injection(of store: LocalStore) -> Injection {
        ...
        case .addedSkills:
            return Injection(parameterLabel: "skillSelectionStore", typeName: "SkillSelectionStore")
        }
    }
    ...
    ///
    /// **The same gap covers the store constructions beside it and is not closed here.**
    ...
    func onlyTheShippedConstantsTestsNameAStoresRealLocation() throws {
        ...
        let permitted: Set<String> = [
            ...
            // That the added-skills file is the one the classification and the wipe both name
            // (SONNY-452). Reads the URL; the store tests beside it write only under a temp root.
            "MacAgentCoreTests/SkillSelectionStoreTests.swift",
            ...
            // which is how a new store is refused until it is classified.
