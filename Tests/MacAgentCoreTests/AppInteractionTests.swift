...
private final class ScriptedChooser: AppInteractionStepChoosing, @unchecked Sendable {
    ...
    static func competent(goal: AppInteractionGoal, screen: AppInteractionScreen) -> AppInteractionModelDecision {
        ...
        if let box = screen.candidates.first(where: { $0.kind == "text area" }),
           screen.context.contains(target) {
            return box.value == goal.text ? .finished : .step(.enterText, ref: box.ref)
        }
        if let row = screen.candidates.first(where: { ($0.kind == "row" || $0.kind == "button") && $0.label.hasPrefix(target + " ·") }) {
            return .step(.press, ref: row.ref)
        }
        if let search = screen.candidates.first(where: { $0.kind == "search field" }), search.value != target {
...
// MARK: - Screen, policy and verification

@Suite
struct AppInteractionScreenTests {
    @Test
    func theModelSeesActionableElementsAndNeverTheMessageHistory() async throws {
        var state = FakeChatAppState(chats: ["Mom", "Dad"], openChat: "Dad")
        state.drafts["Dad"] = "token sk-SECRET here"
        let app = FakeChatAppAccessibility(state: state)
        let snapshot = try await app.observe(processIdentifier: 1, limits: AccessibilityLimits())
        let built = AppInteractionScreenBuilder(redact: { $0.replacingOccurrences(of: "sk-SECRET", with: "[redacted]") })
            .build(from: snapshot, goal: try goal())

        let everything = try String(data: JSONEncoder().encode(built.screen), encoding: .utf8) ?? ""
        #expect(!everything.contains("an earlier private message"))
        #expect(!everything.contains("sk-SECRET"))
        #expect(everything.contains("[redacted]"))
        #expect(built.screen.context == ["Dad"])
        #expect(built.screen.candidates.contains { $0.kind == "row" && $0.label.hasPrefix("Mom ·") })
        #expect(built.screen.candidates.contains { $0.kind == "search field" && $0.can.contains("enter_target") })
    ...
    func labelsAreCutToTheBudget() async throws {
        ...
        let app = FakeChatAppAccessibility(state: FakeChatAppState(chats: [long]))
        let snapshot = try await app.observe(processIdentifier: 1, limits: AccessibilityLimits())
        let built = AppInteractionScreenBuilder(redact: { $0 }).build(from: snapshot, goal: try goal())
...
struct AppInteractionPolicyTests {
    ...
    func aRowNavigatesAndSoDoesARowDrawnAsAButton() async throws {
        let shot = try await snapshot(FakeChatAppState(chats: ["Mom"]))
        ...
        let buttonRow = try element(buttonShot) { $0.role == "AXButton" && $0.parentIndex.map { buttonShot.elements[$0].role == "AXTable" } == true }
        #expect(AppInteractionPolicy.decide(.press, on: buttonRow.id, in: buttonShot, goal: try goal()) == .allow(.press))
    }

    @Test
    func aChatNamedSendStillOpensBecauseARowsTextIsNotItsOwnName() async throws {
        var state = FakeChatAppState(chats: ["Send"])
        state.rowRole = "AXButton"
        let shot = try await snapshot(state)
        let row = try element(shot) { $0.role == "AXButton" }
        #expect(AppInteractionPolicy.decide(.press, on: row.id, in: shot, goal: try goal()) == .allow(.press))
    }

    @Test
    func aListButtonNamedForACommitOrNamedNothingIsRefused() throws {
        ...
        let shot = AccessibilitySnapshot(
            ...
            elements: [
                ...
                AccessibilityElement(id: id(1), parentIndex: 0, depth: 1, role: "AXTable"),
                AccessibilityElement(id: id(2), parentIndex: 1, depth: 2, role: "AXButton", label: "Delete chat", actions: ["AXPress"]),
                AccessibilityElement(id: id(3), parentIndex: 1, depth: 2, role: "AXButton", label: "Mom", actions: ["AXPress"]),
                AccessibilityElement(id: id(4), parentIndex: 1, depth: 2, role: "AXButton", actions: ["AXPress"]),
            ],
            truncation: nil,
            takenAt: Date(timeIntervalSince1970: 0)
        )
        let g = try goal()
        #expect(AppInteractionPolicy.decide(.press, on: id(2), in: shot, goal: g) == .refuse(.mightCommit))
        #expect(AppInteractionPolicy.decide(.press, on: id(3), in: shot, goal: g) == .allow(.press))
        #expect(AppInteractionPolicy.decide(.press, on: id(4), in: shot, goal: g) == .refuse(.mightCommit))
    }

    @Test
    func typingIsLimitedToTheGoalsOwnWordsAndTheMessageNeverGoesIntoSearch() async throws {
        let shot = try await snapshot(FakeChatAppState(chats: ["Mom"], openChat: "Mom"))
...
struct AppInteractionVerifierTests {
    private func check(_ state: FakeChatAppState, _ goal: AppInteractionGoal) async throws -> AppInteractionVerifier.Result {
        let shot = try await FakeChatAppAccessibility(state: state).observe(processIdentifier: 1, limits: AccessibilityLimits())
        return AppInteractionVerifier.check(shot, goal: goal)
    }
    ...
    @Test
    func withoutAVisibleChatNameTheResultIsUnconfirmedNotSatisfied() async throws {
        var state = FakeChatAppState(chats: ["Mom"], openChat: "Mom")
        state.drafts["Mom"] = "Running late, home by 8"
    ...
    @Test
    func aLongerNameContainingTheTargetDoesNotConfirmIt() async throws {
    ...
    @Test
    func aRowInTheChatListDoesNotConfirmTheChatIsOpen() async throws {
        var state = FakeChatAppState(chats: ["Mom", "Dad"], openChat: "Dad")
        state.drafts["Dad"] = "Running late, home by 8"
        state.exposesHeader = false
        #expect(try await check(state, try goal()) == .targetUnconfirmed)
    }
...
struct AppInteractionRuntimeTests {
    ...
    func whenTheChatNameCannotBeSeenTheReportSaysSo() async throws {
        ...
        state.exposesHeader = false
        let app = FakeChatAppAccessibility(state: state)
        // The competent chooser keys off the header; without one, point it at the box directly.
        ...
        #expect(!report.targetConfirmed)
        #expect(report.summary.contains("couldn't confirm"))
    }
    ...
    @Test
    func aModelThatSaysFinishedEarlyIsCheckedAndTheRunCarriesOn() async throws {
        let app = FakeChatAppAccessibility(state: FakeChatAppState(chats: ["Mom"]))
    ...
    func aWindowThatNeverAppearsFailsWithinTheWait() async throws {
        ...
        let runtime = AppInteractionRuntime(
            accessibility: app,
            chooser: ScriptedChooser(),
            apps: FakeApps(),
        ...
        #expect(await runtime.run(try goal()) == .failed(.noWindow("Chat")))
    }

    @Test
    func losingAccessibilityMidRunStopsWithThatReason() async throws {
        let app = FakeChatAppAccessibility(state: FakeChatAppState(chats: ["Mom"]))
        let chooser = ScriptedChooser(overrides: [{ _ in nil }, { _ in nil }])
        let revoking = RevokingChooser(inner: chooser, app: app, revokeOnCall: 2)
        let runtime = AppInteractionRuntime(
            accessibility: app,
            chooser: revoking,
            apps: FakeApps(),
        ...
        #expect(await runtime.run(try goal()) == .failed(.accessibilityNotGranted))
    }
    ...
    func stoppingAfterTextWasTypedSaysSo() async throws {
        ...
        let task = Task { await runtime.run(g) }
        await gate.waitUntilHanging()
        task.cancel()
        #expect(await task.value == .cancelled(typedSomething: true))
