...
Command Center has its own permission/clarification/failure surface as of branch 10: `CommandCenterAttentionPanel` (`CommandCenterView.swift`), rendered unconditionally by the four pages that host `CommandCenterStorageNotice` and self-gating on its own state. It exists because scheduled routines run with nobody watching the widget and the notification fallback is effectively unused — without it an unattended run needing approval would be silently stuck. It mirrors `FloatingWidgetView`'s state precedence exactly (permission > clarification > failure, failure only once the run stops) so the two surfaces can never disagree, and it is deliberately origin-agnostic so a scheduled run's approval reaches it. `CommandCenterRunningIndicator` is still the separate, compact "something is running" line — render it per page by gating on `viewModel.isRunning || viewModel.isAwaitingApproval`.

The floating widget is unchanged and still shows all three states for **every** task regardless of origin. That redundancy is deliberate, not an oversight: the widget is a permanent on-screen overlay while Command Center is a window that may be closed, so for an unattended run the widget is the more reliable surface for an approval, not the less. `AgentViewModel.hasVisibleWidgetPanel` is the single source of truth for both the widget's panel and `FloatingWidgetWindowController`'s compositing decision — a regression test pins it. Do not origin-gate those three states without revisiting both.
