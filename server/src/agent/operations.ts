...
export const OPERATIONS: readonly OperationSpec[] = [
  ...
  },
  {
    ...
    floor: "observe",
    description: "List the largest files in a folder, skipping links. Changes nothing. The evidence names them with their sizes.",
  },
  {
    name: "zip_largest_files",
    version: 1,
    args: z.strictObject({
      folder: path,
      count: z.number().int().min(1).max(50).optional().describe("How many; 3 when the user didn't say."),
      output_path: path.optional().describe("Where to save the archive; a timestamped name in the folder when omitted."),
    }),
    floor: "create",
    description: "Make a zip archive of the largest files in a folder. The evidence names the archive.",
  },
  {
    name: "find_docx",
    version: 1,
    ...
    floor: "observe",
    description: "List the .docx files in a folder and its subfolders that don't have a PDF yet. Changes nothing.",
  },
  {
    ...
    args: z.strictObject({
      folder: path,
      output_folder: path.optional().describe("An existing folder for the PDFs; beside each .docx when omitted."),
    }),
    floor: "create",
    description: "Convert the .docx files in a folder to PDFs with Microsoft Word, skipping ones that already have a PDF.",
  },
  {
    name: "write_file",
    version: 1,
