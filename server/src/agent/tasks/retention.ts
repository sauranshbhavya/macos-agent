 ...
 * fails tasks nobody has touched for a day, and releases credit holds a process died holding.
 */
...
export async function sweepTasksOnce(deps: RetentionDeps): Promise<SweepOutcome & { releasedHolds: number }> {
  const now = deps.now();
...
export function startTaskRetentionSweeper(
  ...
  const tick = async (): Promise<void> => {
    ...
    try {
      ...
      if (result.deleted + result.abandoned + result.releasedHolds > 0) {
