 ...
 *
 * **The schema comes from `rebuildSchema`, and the two tests that move migrations underneath
 * themselves live in `migration-round-trip.db.test.ts` instead** (SONNY-366, and PR #171's rebase).
 * This file used to carry them and to do its own `DROP SCHEMA` + `up()`; both are now the shared
 * helper's, which is the only door to the migration runner. Splitting them out was not
 * bookkeeping: `schema.test.ts`'s exemption list asks that every entry be a file whose *name* says
 * it is about migrations, and warns that an entry for a file named like this one would be the guard
...
describeDb("an auth operation knows which instance it is acting on", () => {
  ...
  describe("SONNY-365 — a drain discharges the obligation it claimed and no other", () => {
    itUnderHangBackstop("does NOT discharge an obligation created while its own provider call was in flight", async () => {
    ...
    itUnderHangBackstop("services the new obligation with its own provider call in the same run", async () => {
    ...
    itUnderHangBackstop("does NOT discharge an obligation created by a SUPERSESSION during the call", async () => {
    ...
    itUnderHangBackstop("does NOT discharge a SIBLING row's fresh obligation, which no claim ever covered", async () => {
    ...
    itUnderHangBackstop("increments the episode on each of the three events that start one, and never otherwise", async () => {
    ...
    itUnderHangBackstop("still discharges normally when nothing interleaves", async () => {
  ...
  describe("SONNY-353 — the latest code at a mailbox is the last one issued", () => {
    ...
    itUnderHangBackstop("consumes the LAST of several issuances that share one instant", async () => {
    ...
    itUnderHangBackstop("discloses the failure of the LAST issuance when two share an instant", async () => {
    ...
    itUnderHangBackstop("orders by issuance even when the clock goes backwards", async () => {
    ...
    itUnderHangBackstop("refuses an issue_seq written by application code", async () => {
